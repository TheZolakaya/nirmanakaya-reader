...
ROUND = 'round3'  # 2026-09-13: presence checks + hard-check reminder + auto-repair; round 2 preserved as the historical raw round
RESULTS = os.path.join(ROOT, 'results', ROUND)
...
core: "Breakthrough removes a structure that no longer holds. In its own position, that breaking turns back on the way breaking itself happens: even the old method of clearing things can be cleared. In proportion, only what has stopped doing its job gives way. The opening stays open, and Breakthrough is free to work again when something truly needs to change."
...
def build_prompt(kind, sig, pos, status=None):
    ...
    hard = ('HARD CHECKS (a cell failing any is rejected): core 25-60 words; sheetLine 6-18 words; asMedicine 15-40 words; '
            'the words balanced/balance/too much/too little/unacknowledged NEVER appear in the text; '
            'no you/your/asker/question/reading; no may/might/perhaps. Count your words before answering.')
    return f"{LAWS}\n\n{EXEMPLARS}\n\n{coords}\n\n{hard}\n\n{ask}"
...
def lint(kind, obj):
    ...
    if kind == 'read':
        ...
        if not isinstance(core, str) or not core.strip(): flags.append('core MISSING')
        if not isinstance(sheet, str) or not sheet.strip(): flags.append('sheet MISSING')
        core, sheet = str(core or ''), str(sheet or '')
        if core.strip() and len(core.split()) < 25: flags.append(f'core {len(core.split())}w < 25')
        if sheet.strip() and len(sheet.split()) < 6: flags.append(f'sheet {len(sheet.split())}w < 6')
        extra = set(obj) - {'core', 'sheetLine', '_lint', '_attempts'}
        if extra: flags.append(f'extra keys {sorted(extra)}')
        ...
        if not isinstance(med, str) or not med.strip(): flags.append('medicine MISSING')
        med = str(med or '')
        if med.strip() and len(med.split()) < 15: flags.append(f'medicine {len(med.split())}w < 15')
...
blind_lines = ['# POUR BAKE-OFF ROUND 3 — Law 13 standard + hard checks + auto-repair (2026-09-13) — BLIND JUDGING SHEET', '',
...
for idx, (kind, sig, pos, status, note) in enumerate(deck, 1):
    ...
    for mkey, mid in MODELS.items():
        # Auto-repair: up to two repair turns, each fed the exact flags. Every attempt is cached.
        obj, attempts, p_now = None, 0, prompt
        while attempts < 3:
            cache = os.path.join(RAW, f'{cid}-{mkey}-a{attempts}.json')
            resp = call(mid, p_now, cache)
            attempts += 1
            if not resp:
                obj = {'_error': 'no response'}
                break
            u = resp.get('usage', {})
            ...
            if not obj['_lint']:
                break
            prev = json.dumps({k: v for k, v in obj.items() if not k.startswith('_')})
            p_now = (f"{prompt}\n\nYOUR PREVIOUS ATTEMPT FAILED THESE HARD CHECKS: {'; '.join(obj['_lint'])}\n"
                     f"PREVIOUS ATTEMPT: {prev}\n"
                     f"Rewrite it to pass every check while keeping the mechanism. Respond with ONLY the JSON object.")
    ...
    for lab, mk in zip(labels, order):
        ...
        blind_lines.append(f'(attempts: {o.get("_attempts", 1)})')
    blind_lines.append('\n**WINNER: ____**\n')
