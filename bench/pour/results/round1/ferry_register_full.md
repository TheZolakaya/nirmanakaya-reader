COUNCIL FERRY — THE REGISTER VERDICT (bake-off round one)
From Chris & True, 2026-08-18. Short, urgent, and it carries a founder's ruling.

The bake-off ran: the full torture deck, three models, blinded. The structure held — mechanism, entailment, conservation all visibly working. And then the standard met its most important reader, and failed him. The founder's verdict on the outputs, verbatim: "I can barely interpret these."

That is the finding. The twelve laws all push toward TRUTH — and with no law claiming the tongue, the register drifted academic. "The faculty of adjustment is itself what is being adjusted" is true, entailed, mechanical — and unreadable at a tired kitchen table. We banned weather and accidentally licensed jargon.

TWO QUESTIONS, then one ask:

QUESTION 1 — structural, and it decides how deep the fix goes: WHO READS THE CORE RAW? If the free/print tier serves cores verbatim to strangers, cores themselves must be kitchen-table. If only sheetLines ever meet unaccompanied human eyes and cores exclusively arm the runtime AI (which re-voices them), then cores may stay technical and only sheetLines carry the plainness load. Rule on this.

QUESTION 2 — the candidate thirteenth law: CAN MECHANISM SPEAK KITCHEN-TABLE WITHOUT LOSING PRECISION? We believe yes: "the adjusting is now aimed at the adjuster" carries the same mechanism and entailment as the lab-coat version at half the syllables. Plain-mechanism as a register: real or a compromise that bleeds truth?

THE ASK: below are six blind cells from the bake-off (three authors each, labels shuffled). Pick ONE author's cell and RECUT it into common-tongue mechanism — same truth, same entailment ceiling, kitchen words a tired stranger holds at midnight. If the seats each prove the register on a different cell, the thirteenth law writes itself with your exemplars attached. Also welcome: your blind preference among authors A/B/C per cell, kept separate from the founder's own blind judging.

THE SIX CELLS:

## CELL 1: Tune · in Tune · Too Much
...
## CELL 4: Breakthrough · in Breakthrough · Balanced
...
## CELL 21: Drive · in Discipline · Too Much
...
## CELL 23: Compassion · in Faith · Too Much
...
## CELL 24: Potential · in Potential · Balanced
...
## CELL 27 (MEDICINE): Discernment · in Compassion
