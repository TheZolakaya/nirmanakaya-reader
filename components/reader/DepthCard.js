...
// V3: Only two states — collapsed or showing content
const DEPTH = {
  COLLAPSED: 'collapsed',
  SHALLOW: 'shallow',  // V3: maps to "showing content" (kept for compat)
  SURFACE: 'surface',
  WADE: 'wade',
  SWIM: 'swim',
  DEEP: 'deep'
...
// V3: Normalize flat API response to legacy depth-tiered shape
// Handles both V3 (flat: summary/reading/why/rebalancer/growth as strings)
// and legacy (tiered: shallow/wade/swim/deep as separate strings) data
const normalizeCardData = (data) => {
  if (!data) return data;
  // V3 shape: has 'reading' field, no 'shallow' field
  if (data.reading !== undefined && data.shallow === undefined) {
    const n = { ...data, summary: data.summary || '' };
    // Map flat reading to all depth tiers so existing getContent() works
    n.shallow = data.reading;
    n.wade = data.reading;
    n.swim = data.reading;
    n.deep = data.reading;
    // Normalize why from flat string to depth-tiered object
    if (typeof data.why === 'string') {
      n.why = { shallow: data.why, wade: data.why, swim: data.why, deep: data.why };
    }
    // Normalize rebalancer from flat string to depth-tiered object
    if (typeof data.rebalancer === 'string') {
      n.rebalancer = { shallow: data.rebalancer, wade: data.rebalancer, swim: data.rebalancer, deep: data.rebalancer };
    }
    // Normalize growth from flat string to depth-tiered object
    if (typeof data.growth === 'string') {
      n.growth = { shallow: data.growth, wade: data.growth, swim: data.growth, deep: data.growth };
    }
    return n;
  }
  // Legacy shape — ensure summary field exists
...
const DepthCard = ({
  ...
  const getContent = (d) => {
    // V1: Four-tier depth progression — shallow/wade/swim/deep
    // Shallow = plain reading (no architecture), wade adds Gestalt/Portal,
    // swim adds horizon/interaction, deep adds full geometry
    switch (d) {
      case DEPTH.SHALLOW:
      case DEPTH.SURFACE:
        if (hasContentValue(cardData.shallow)) return cardData.shallow;
  ...
  const handleCardClick = () => {
    if (depth === DEPTH.COLLAPSED) {
      setDepth(DEPTH.SHALLOW); // V1: First tap goes to Shallow (plain reading, no architecture)
  ...
  const toggleCollapse = (e) => {
    e.stopPropagation();
    if (depth === DEPTH.COLLAPSED) {
      setDepth(DEPTH.SHALLOW); // V1: First tap goes to Shallow
      // Trigger on-demand load if content not yet fetched
      if (isNotLoaded && onRequestLoad) {
        onRequestLoad();
      }
  ...
  const getFirstRebalancerDepth = () => {
    ...
    // Shallow is the default starting depth
    if (hasContentValue(r.shallow)) return DEPTH.SHALLOW;
  ...
  const getGrowthContent = (d) => {
    ...
    switch (d) {
      case DEPTH.SHALLOW:
        if (hasContent(g.shallow)) return g.shallow;
        if (hasContent(g.surface)) return g.surface;
        return hasContent(g.wade) ? g.wade : null;
      case DEPTH.SURFACE:
        return hasContent(g.surface) ? g.surface : null;
      case DEPTH.WADE:
        return hasContent(g.wade) ? g.wade : (hasContent(g.shallow) ? g.shallow : null);
      case DEPTH.SWIM:
        return hasContent(g.swim) ? g.swim : null;
      case DEPTH.DEEP:
        return hasContent(g.deep) ? g.deep : null;
      default:
        return hasContent(g.shallow) ? g.shallow : null;
    }
  };

  // Find first available growth depth
  const getFirstGrowthDepth = () => {
    const g = cardData.growth;
    if (!g) return DEPTH.SHALLOW;
    if (hasContentValue(g.shallow)) return DEPTH.SHALLOW;
    if (hasContentValue(g.wade)) return DEPTH.WADE;
    ...
    <div className={`content-pane rounded-lg border-2 p-5 mb-5 transition-all duration-300 ${getSectionStyle()}`}>
      ...
      {depth !== DEPTH.COLLAPSED && (
        <>
          ...
          {/* V3 Summary — 2-sentence anchor at top of card */}
          {cardData.summary && !isLoading && (
            <div className="text-sm text-amber-200/90 font-medium mb-4 pb-3 border-b border-amber-500/20 leading-relaxed">
              {cardData.summary}
            </div>
          )}
        ...
        <div className="mt-4 ml-4 rounded-lg border-2 border-emerald-500/30 bg-emerald-950/20 p-4">
          ...
          {rebalancerDepth !== DEPTH.COLLAPSED && (
            <>
              ...
              {!isFirstContact && rebalancerDepth !== DEPTH.COLLAPSED && cardData.rebalancer?.architecture && (
                <ArchitectureBox
        ...
        <div className="mt-4 ml-4 rounded-lg border-2 border-teal-500/30 bg-teal-950/20 p-4">
          ...
          {growthDepth !== DEPTH.COLLAPSED && (
            <>
              ...
              {!isFirstContact && growthDepth !== DEPTH.COLLAPSED && cardData.growth?.architecture && (
                <ArchitectureBox
        ...
        <div className="mt-4 rounded-lg border-2 border-cyan-500/30 bg-cyan-950/20 p-4">
          ...
          {!isWhyCollapsed && (
            <>
              ...
              {/* WORDS TO THE WHY — V3: single depth, no tier navigation */}
              {cardData.why && (cardData.why.shallow || cardData.why.wade || cardData.why.swim || cardData.why.deep) && (
                <div className="border-t border-cyan-700/30 pt-4">
                  ...
                  {/* V3: WHY Content — single depth */}
                  <div className="text-sm text-cyan-100/90 mb-4">
                    {(() => {
                      const whyContent = cardData.why?.shallow || cardData.why?.wade || cardData.why?.swim || cardData.why?.deep || null;
                      return whyContent ? (
                        <div className="space-y-3">
