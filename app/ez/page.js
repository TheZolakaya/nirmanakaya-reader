...
FIND IT. The person tapped the chip YOU wrote ("help me find it"). The thing you named in that chip was: "${loc.what}" — those are YOUR words from your last turn, not theirs; never quote them back as something the person said. Their words are only what they typed. This is narrowing turn ${loc.step}${loc.balanced ? ' (a Balanced card: ONE narrowing turn at most)' : ' (three at most)'}. The draw cannot name the thing; only they can. Your job is to NARROW, one question per turn, with the question coming from the geometry; and to STOP the moment they have named it.
