...
const ANNOTATION_FIELDS = `
  ...
  annotation_type, selected_text, text_prefix, text_suffix, mentioned_user_ids, hashtags,
...
export async function createAnnotation(chapterSlug, content, isPublic = true, hashtags = []) {
...
export async function createInlineAnnotation(chapterSlug, content, selectedText, textPrefix, textSuffix, mentionedUserIds = [], hashtags = []) {
  return createAnnotationFull({
    chapterSlug,
    content,
    isPublic: true,
...
async function createAnnotationFull({ chapterSlug, content, isPublic, annotationType, selectedText, textPrefix, textSuffix, mentionedUserIds, hashtags }) {
...
/** Get annotations by hashtag (cross-chapter) */
export async function getAnnotationsByHashtag(hashtag) {
  if (!supabase || !hashtag) return [];

  const { data, error } = await supabase
    .from('book_annotations')
    .select(ANNOTATION_FIELDS + ', chapter_slug')
    .eq('is_public', true)
...
/** Search existing hashtags for autocomplete */
export async function searchHashtags(query) {
  if (!supabase || !query || query.length < 1) return [];

  // Query distinct hashtags from recent annotations
  const { data, error } = await supabase
    .from('book_annotations')
    .select('hashtags')
    .eq('is_public', true)
  ...
  // Flatten, dedupe, and filter by query
  const allTags = new Set();
  (data || []).forEach(row => {
    (row.hashtags || []).forEach(tag => allTags.add(tag));
  });
