...
export const VERDICTS = {
  // Labels carry the TEMPORAL FRAME (founder, 2026-08-12): the verdict reads the
  // current vector — "as it stands" — never the future. A bare "No" chip read as
  // a court ruling on the dream; "Not as it stands" is the mirror talking.
  YES_AND: { label: 'Yes, as it stands — and', polarity: 'yes', tone: 'emerald' },
  YES_BUT: { label: 'Yes, as it stands — but', polarity: 'yes', tone: 'emerald' },
  NO_BUT: { label: 'Not as it stands — but', polarity: 'no', tone: 'rose' },
  NO_AND: { label: 'Not as it stands — and', polarity: 'no', tone: 'rose' },
  ...
  // NAMED — NAME-shaped questions ("what have I been carrying...") ask the field to
  // IDENTIFY something. The headline must grammatically complete the question in the
  // asker's own coin (their verb, their metaphor) — never a condition report.
  NAMED: { label: 'The Answer', polarity: 'named', tone: 'sky' }
};
...
export function typeQuestion(question) {
  ...
  // Condition-shaped: "how is/are things going" — a status check, answered on the
  // WELL/STRAINED grid. Distinct from NAME-shaped: "what have I been carrying",
  // "why does this keep happening" — asks the field to IDENTIFY something, and a
  // condition report ("Going well — but") answers the wrong question (2026-08-12,
  // caught live on the founder's own reading).
  const conditionShaped = /^how('s| is| are| am| was| were| has| have)\b/i.test(q);
  ...
  if (conditionShaped) return { class: 'STATE', signals: [...signals, 'condition-shaped: how-is status check'], hardGate: false };
  if (stateShaped && !/^should\b/i.test(q)) return { class: 'NAME', signals: [...signals, 'name-shaped: asks to identify something'], hardGate: false };
...
WALK REGISTER LAW (2026-08-19): each walk entry is a RECEIPT, not an essay — at most ~35 words, plain speech, written to be read by the asker. Never paste step names or labels in capitals; never repeat the dossier wholesale — cite only the fact you actually used.
...
- TEMPORAL FRAME: every verdict reads the PRESENT configuration — the current vector — never the future. A NO means "not as it stands," never "never." When the aim is achievable, the qualifier presents the medicine as how the vector turns toward it; NO_AND is reserved for configurations where the field shows no path at all. The headline and qualifier must speak in this frame: mirror language ("as things stand," "the current vector," "from here"), never prophecy language ("this won't work," "you can't").
...
- LEAN ARITHMETIC: if you cite the lean's numbers, reproduce them EXACTLY from its components array — three components (status 0.6, displacement 0.3, resonance 0.1), renormalized over the computable ones. Never restate weights from memory or collapse to two components: a wrong explanation of a right number is still a violation (2026-08-14 audit: the number was correct; a narrated "60/40" was not).
...
${HARD_RULES}
${typerResult.class === 'NAME' ? `
NAME-CLASS CONTRACT (this question asks the field to IDENTIFY something — not to rule yes/no, not to report condition):
- verdict MUST be "NAMED".
- The headline MUST grammatically complete the asker's question, in the asker's own COIN: reuse their verb and their metaphor. "What have I been carrying?" is answered by naming the carried thing in carrying language ("The work is complete. You've been carrying it past the finish line."). Never answer a what-question with a status report.
- Common tongue ONLY in headline and qualifier: no signature names, no statuses, no house/channel/architecture vocabulary. Speak as a wise friend across the table. Full provenance goes in the walk and qualifierSource, where it belongs.
- The named thing must be sourced from the computed geometry — the over/under-expressed signature, its rebalancer, the pattern — translated into plain words. You are naming what the geometry names, not inventing.
- qualifier: one plain sentence of nuance or medicine, also in the asker's coin.
` : ''}
${EXEMPLARS}
...
Respond with ONLY a JSON object, no markdown fences, matching exactly:
{
  ...
  "typerClass": "CLEAR | COMPOUND | HIGH_STAKES | STATE | NAME | DISPLACED_PREMISE",
  "verdict": "YES_AND | YES_BUT | NO_BUT | NO_AND | WELL_AND | WELL_BUT | STRAINED_BUT | STRAINED_AND | COUNSEL | NAMED | REFRAME | CONDITIONS | NONE_CHAIN",
  "counsel": null,
...
6. Select the most supported branch — UNLESS the predictive/outcome/high-stakes rules apply (then CONDITIONS: no winner, decision returned) — or, if every branch reads badly, NONE_OF_THESE: what do all the branches share? That shared condition is the real finding.
...
- PREDICTIVE GUARD: if the question asks about a FUTURE or EXTERNAL outcome ("are we going to...", "will we have to...", "can we afford..."), the field CANNOT answer it — it reads the asker's present configuration only. Say so in the headline explicitly, and do NOT crown a winner as if forecasting.
- OUTCOME vs ACTION: if the options are outcomes that might BEFALL the asker (a hope vs a fear, things done TO them) rather than actions they would TAKE, do not select between them — render CONDITIONS: read each branch as the asker's present relationship to that possibility, and hand the question of what happens back to the facts and the asker.
- HIGH STAKES: if the question class is HIGH_STAKES (home, family, marriage, livelihood, irreversibles), render CONDITIONS — read all branches fully, name each branch's medicine, but NO selection. The decision is explicitly returned. A crowned winner on an irreversible is a command wearing a ranking.
...
Respond with ONLY a JSON object, no markdown fences, matching exactly:
{
  ...
  "verdict": "CHOICE | NONE_OF_THESE | CONDITIONS",
  "selection": "the selected option's exact text, or null if NONE_OF_THESE or CONDITIONS",
  "headline": "the answer in one plain sentence, 20 words max",
...
REGISTER LAW (applies to every item, all modes — 2026-08-14, after a specimen where the
machinery flooded the surface): each item's "statement" is COMMON TONGUE ONLY — a sentence
a tired person can stand in. No signature names, no statuses, no house/channel/stage/
dimension vocabulary, no displacement or geometry terms in the statement itself. ALL of
that belongs in the "source" field and the walk, where provenance lives. If a statement
reads like the architecture talking to itself, it is a violation — translate it.
GOOD: "Something already finished is still being carried as if it were unfinished."
BAD: "Reciprocity operates in the Body at Fruition, in Outer dimension, through Polarity."
