# Appendix B: Reference Tables

---

## Table 1: The Twenty-Two Archetypal Signatures

| Position | Name | Verb | House | Channel | Element | Process Stage | Governs | Governed By |
|----------|------|------|-------|---------|---------|---------------|---------|-------------|
| 0 | Potential | EXISTS | Gestalt | Intent | Fire | Seed | Spirit House | Source (10) |
| 1 | Will | DIRECTS | Gestalt | Structure | Earth | Feedback | Body House | Source (10) |
| 2 | Wisdom | DISCERNS | Spirit | Cognition | Air | Bridge | — | Potential (0) |
| 3 | Nurturing | TENDS | Spirit | Structure | Earth | Feedback | — | Potential (0) |
| 4 | Order | STRUCTURES | Mind | Intent | Fire | Seed | — | Actualization (19) |
| 5 | Culture | SHARES | Mind | Resonance | Water | Fruition | — | Actualization (19) |
| 6 | Compassion | CONNECTS | Emotion | Resonance | Water | Seed | — | Recognition (20) |
| 7 | Drive | MOVES | Emotion | Intent | Fire | Fruition | — | Recognition (20) |
| 8 | Fortitude | ENDURES | Body | Structure | Earth | Fruition | — | Will (1) |
| 9 | Discipline | FOCUSES | Body | Cognition | Air | Feedback | — | Will (1) |
| **10** | **Source** | **TURNS** | **Portal** | **—** | **—** | **—** | **Gestalt House** | **—** |
| 11 | Equity | CALIBRATES | Body | Resonance | Water | Seed | — | Will (1) |
| 12 | Sacrifice | RELEASES | Body | Intent | Fire | Bridge | — | Will (1) |
| 13 | Change | TRANSFORMS | Emotion | Structure | Earth | Bridge | — | Recognition (20) |
| 14 | Tune | REGULATES | Emotion | Cognition | Air | Feedback | — | Recognition (20) |
| 15 | Abstraction | PATTERNS | Mind | Cognition | Air | Fruition | — | Actualization (19) |
| 16 | Breakthrough | CLEARS | Mind | Structure | Earth | Feedback | — | Actualization (19) |
| 17 | Inspiration | ILLUMINATES | Spirit | Intent | Fire | Seed | — | Potential (0) |
| 18 | Imagination | ENVISIONS | Spirit | Resonance | Water | Bridge | — | Potential (0) |
| 19 | Actualization | BECOMES | Gestalt | Cognition | Air | Fruition | Mind House | Source (10) |
| 20 | Recognition | INTEGRATES | Gestalt | Resonance | Water | Bridge | Emotion House | Source (10) |
| **21** | **Creation** | **COMPLETES** | **Portal** | **—** | **—** | **—** | **—** | **—** |

---

## Table 2: The Five Houses

| House | Element | Domain | Archetypes | Governed By | House Command |
|-------|---------|--------|------------|-------------|---------------|
| Gestalt | Aether | Coherence, identity, unified observer | 0, 1, 19, 20 | Source (10) | "Fulfill Your Destiny" |
| Spirit | Fire | Orientation, aim, directed will | 2, 3, 17, 18 | Potential (0) | "Witness Creation" |
| Mind | Air | Modeling, processing, abstraction | 4, 5, 15, 16 | Actualization (19) | "Channel the Force" |
| Emotion | Water | Valuation, connection, felt meaning | 6, 7, 13, 14 | Recognition (20) | "Free Will" |
| Body | Earth | Manifestation, form, embodied action | 8, 9, 11, 12 | Will (1) | "Uphold the Law" |

---

## Table 3: Process Cycles by House

Each house achieves its own element at a specific process stage and cycles through all four elements in a unique order.

| House | Seed | Bridge | Fruition | Feedback | Element Achieved At |
|-------|------|--------|----------|----------|---------------------|
| Gestalt | Fire (0) | Water (20) | Air (19) | Earth (1) | Seed |
| Spirit | Air (2) | Earth (3) | Fire (17) / Water (18) | — | Fruition |
| Mind | Fire (4) | — | Water (5) / Air (15) | Earth (16) | Fruition |
| Emotion | Water (6) | Earth (13) | Fire (7) | Air (14) | Seed |
| Body | Water (11) | Fire (12) | Earth (8) | Air (9) | Fruition |

---

## Table 4: The Four Channels

| Channel | Element | Suit | Mode | Question |
|---------|---------|------|------|----------|
| Intent | Fire | Wands | Directed will, initiating force | WHO |
| Cognition | Air | Swords | Patterning, analysis, modeling | HOW |
| Resonance | Water | Cups | Felt meaning, valuation, connection | WHAT |
| Structure | Earth | Pentacles | Form, manifestation, grounding | WHERE/WHEN |

---

## Table 5: The Four Process Stages

| Stage | Quality | Yang/Yin | Character |
|-------|---------|----------|-----------|
| Seed | Yang-pure | Initiating | Where potential first differentiates into specific intent |
| Bridge | Yin-in-Yang | Developing | Where the seed encounters relationship and begins to elaborate |
| Fruition | Yin-pure | Completing | Where the planted seed reaches its full expression |
| Feedback | Yang-in-Yin | Integrating | Where the cycle folds back and feeds what was learned into the next beginning |

---

## Table 6: The Seven Rings

| Ring | Name | Structural View | Teleological View |
|------|------|----------------|-------------------|
| 0 | The Unnameable Ground | Epistemic entry / Ontological destination | Where derivation begins and experience returns |
| 1 | Source (Ingress) | The portal through which consciousness enters | The threshold of individuation |
| 2 | Gestalt (Identity) | The self that persists across change | The minimum viable identity |
| 3 | Body (Structure) | Manifestation and form | Directive Fields — the 20 Spiral Operators |
| 4 | Emotion (Felt Resonance) | Valuation and connection | Spiral Messengers — guides, synchronicity |
| 5 | Mind (Focal Agency) | Modeling and pattern | Incarnated Consciousness — the agent under the Veil |
| 6 | Spirit (Emergent Direction) | Orientation and aim | Reflected Systems — culture, religion, technology |
| 7 | Creation (Egress/Residue) | Where unfinished creation goes | Creative exhaust awaiting reintegration |

---

## Table 7: Duality Pairs

### Vertical Pairs (Sum = 20)

Same identity across two horizons. Corrects Too Little states.

| Creation Horizon | Operation Horizon | Sum |
|-----------------|-------------------|-----|
| 2 (Wisdom) | 18 (Imagination) | 20 |
| 3 (Nurturing) | 17 (Inspiration) | 20 |
| 4 (Order) | 16 (Breakthrough) | 20 |
| 5 (Culture) | 15 (Abstraction) | 20 |
| 6 (Compassion) | 14 (Tune) | 20 |
| 7 (Drive) | 13 (Change) | 20 |
| 8 (Fortitude) | 12 (Sacrifice) | 20 |
| 9 (Discipline) | 11 (Equity) | 20 |

### Diagonal Pairs (Sum = 19 or 21)

Complementary force across polarity. Corrects Too Much states.

| Pair A | Pair B | Sum |
|--------|--------|-----|
| 2 (Wisdom) | 17 (Inspiration) | 19 |
| 3 (Nurturing) | 18 (Imagination) | 21 |
| 4 (Order) | 15 (Abstraction) | 19 |
| 5 (Culture) | 16 (Breakthrough) | 21 |
| 6 (Compassion) | 13 (Change) | 19 |
| 7 (Drive) | 14 (Tune) | 21 |
| 8 (Fortitude) | 11 (Equity) | 19 |
| 9 (Discipline) | 12 (Sacrifice) | 21 |

### Reduction Pairs (Shared Digit Sum)

Maximally distant on the tesseract. Corrects Unacknowledged states.

| Archetype A | Archetype B | Digit Sum |
|-------------|-------------|-----------|
| 2 (Wisdom) | 11 (Equity) | 2 |
| 3 (Nurturing) | 12 (Sacrifice) | 3 |
| 4 (Order) | 13 (Change) | 4 |
| 5 (Culture) | 14 (Tune) | 5 |
| 6 (Compassion) | 15 (Abstraction) | 6 |
| 7 (Drive) | 16 (Breakthrough) | 7 |
| 8 (Fortitude) | 17 (Inspiration) | 8 |
| 9 (Discipline) | 18 (Imagination) | 9 |

---

## Table 8: The Four Transient States

| State | Character | Temporal Association | Correction Path | Correction Type |
|-------|-----------|---------------------|-----------------|-----------------|
| Balanced | Functioning as designed | Now | None needed | — |
| Too Much | Over-expression, grasping | Future | Diagonal partner (sum 19/21) | Complementary force |
| Too Little | Under-expression, withdrawing | Past | Vertical partner (sum 20) | Reconnection |
| Unacknowledged | Operating but unseen | Shadow/Absence | Reduction partner (shared digit sum) | Illumination |

---

## Table 9: The Forty Bounds

### Intent Channel (Wands/Fire)

| # | Nirmanakaya Name | Associated Archetype | Range | Interpretation |
|---|-----------------|---------------------|-------|----------------|
| Ace | Activation | Actualization (19) | Inner | Pure potential in Intent |
| 2 | Orientation | Imagination (18) | Inner | Visionary direction-setting |
| 3 | Assertion | Order (4) | Inner | Structure asserting outward |
| 4 | Alignment | Drive (7) | Inner | Momentum in harmonious alignment |
| 5 | Offering | Sacrifice (12) | Inner | Purposeful release in action |
| 6 | Recognition | Sacrifice (12) | Outer | Release acknowledged by others |
| 7 | Resolve | Drive (7) | Outer | Momentum defending position |
| 8 | Command | Order (4) | Outer | Structure in swift execution |
| 9 | Resilience | Imagination (18) | Outer | Vision persisting through challenge |
| 10 | Realization | Actualization (19) | Outer | Becoming fully carried |

### Cognition Channel (Swords/Air)

| # | Nirmanakaya Name | Associated Archetype | Range | Interpretation |
|---|-----------------|---------------------|-------|----------------|
| Ace | Perception | Actualization (19) | Inner | Clarity initiating |
| 2 | Reflection | Wisdom (2) | Inner | Discernment weighing options |
| 3 | Calculation | Abstraction (15) | Inner | Logic cutting through |
| 4 | Dissonance | Tune (14) | Inner | Regulation in stillness |
| 5 | Clash | Discipline (9) | Inner | Focused practice in conflict |
| 6 | Guidance | Discipline (9) | Outer | Focused practice directing movement |
| 7 | Reconciliation | Tune (14) | Outer | Regulation through strategy |
| 8 | Absorption | Abstraction (15) | Outer | Logic at its limit |
| 9 | Multiplicity | Wisdom (2) | Outer | Discernment holding many patterns |
| 10 | Realization | Actualization (19) | Outer | Clarity fully realized |

### Resonance Channel (Cups/Water)

| # | Nirmanakaya Name | Associated Archetype | Range | Interpretation |
|---|-----------------|---------------------|-------|----------------|
| Ace | Receptivity | Recognition (20) | Inner | Integrative recognition opening |
| 2 | Merge | Compassion (6) | Inner | Relational connection meeting |
| 3 | Ripple | Culture (5) | Inner | Shared meaning celebrating |
| 4 | Drift | Tune (14) | Inner | Regulation in contemplation |
| 5 | Ache | Equity (11) | Inner | Fair calibration feeling loss |
| 6 | Reciprocity | Equity (11) | Outer | Fair calibration in exchange |
| 7 | Curation | Tune (14) | Outer | Regulation amid possibility |
| 8 | Release | Culture (5) | Outer | Shared meaning moving on |
| 9 | Fulfillment | Compassion (6) | Outer | Relational connection satisfied |
| 10 | Completion | Recognition (20) | Outer | Integrative recognition fulfilled |

### Structure Channel (Pentacles/Earth)

| # | Nirmanakaya Name | Associated Archetype | Range | Interpretation |
|---|-----------------|---------------------|-------|----------------|
| Ace | Definition | Will (1) | Inner | Directed agency beginning |
| 2 | Tune | Nurturing (3) | Inner | Tending in dynamic flow |
| 3 | Formation | Breakthrough (16) | Inner | Clearing creating form |
| 4 | Preservation | Change (13) | Inner | Transformation holding firm |
| 5 | Endurance | Fortitude (8) | Inner | Endurance through hardship |
| 6 | Support | Fortitude (8) | Outer | Endurance shared |
| 7 | Harvest | Change (13) | Outer | Transformation assessing growth |
| 8 | Commitment | Breakthrough (16) | Outer | Clearing through dedication |
| 9 | Flourishing | Nurturing (3) | Outer | Tending in abundance |
| 10 | Achievement | Will (1) | Outer | Directed agency fully manifest |

---

## Table 10: The Sixteen Agents

Agents are the externalized behavioral expressions of the sixteen manifest archetypes (positions 2-9, 11-18). Gestalt archetypes and portals do not produce agents.

| Role | Intent (Fire) | Cognition (Air) | Resonance (Water) | Structure (Earth) |
|------|--------------|-----------------|-------------------|-------------------|
| **Initiate** | Inspiration (17) | Wisdom (2) | Imagination (18) | Nurturing (3) |
| **Catalyst** | Order (4) | Abstraction (15) | Culture (5) | Breakthrough (16) |
| **Steward** | Drive (7) | Tune (14) | Compassion (6) | Change (13) |
| **Adept** | Sacrifice (12) | Discipline (9) | Equity (11) | Fortitude (8) |

---

## Table 11: The Forty-Fold Seal Grid

```
         Spirit  Emotion  Mind   Body
        ──────  ───────  ────  ─────
Fire       17       7      4     12    = 40
Air         2      14     15      9    = 40
Water      18       6      5     11    = 40
Earth       3      13     16      8    = 40
        ──────  ───────  ────  ─────
           40      40     40     40
```

All rows sum to 40. All columns sum to 40. All sixteen toroidal 2x2 blocks sum to 40. Four corners sum to 40.

---

## Table 12: The Pentagram House Mapping

```
            Gestalt (Aether)
               ★
              / \
             /   \
            /     \
     Mind ★       ★ Emotion
    (Air)  \     /  (Water)
            \   /
             \ /
     Spirit ★───★ Body
    (Fire)        (Earth)
```

Gestalt at apex. Spirit and Body as feet. Mind and Emotion as arms. The upright pentagram represents consciousness properly oriented. The trace order determines the Seal's column sequence.

---

## Table 13: The Quadraverse Correspondences

| Dimension | WHO (Spirit/Fire) | HOW (Mind/Air) | WHAT (Emotion/Water) | WHERE-WHEN (Body/Earth) |
|-----------|-------------------|----------------|---------------------|------------------------|
| Force | Strong Nuclear | Weak Nuclear | Electromagnetic | Gravity |
| State of Matter | Plasma | Gas | Liquid | Solid |
| DNA Base | Guanine | Cytosine | Adenine | Thymine |
| Dimension | Time | Height | Width | Depth |
| Process Stage | Seed | Bridge | Fruition | Feedback |
| Channel | Intent | Cognition | Resonance | Structure |
| Element | Fire | Air | Water | Earth |

---

## Table 14: The Complete 78 Signature Count

| Category | Count | Description |
|----------|-------|-------------|
| Archetypal Signatures | 20 | Positions 0-9, 11-20 (the verbs of consciousness) |
| Portal Signatures | 2 | Source (10), Creation (21) — thresholds, not archetypes |
| Bounds | 40 | 20 archetypes × 2 polarities (Inner/Outer) |
| Agents | 16 | 16 manifest archetypes × 1 externalized expression |
| **Total** | **78** | **The complete vocabulary of conscious experience** |

Note: Gestalt archetypes (0, 1, 19, 20) have bounds but no agents. Portals (10, 21) have neither bounds nor agents.

