...
function computeHealth(aggregates, governance, flags, wheelWorldFields, correctiveVector) {
  ...
  // Base score: percentage of balanced positions
  const balancedRatio = aggregates.statuses.Balanced / total;

  // Governance penalty: imbalanced governors affect their entire house
  // Weighted by house size — a sick governor over 4 positions is worse than over 2
  let governancePenalty = 0;
  for (const gov of governance.governors) {
    if (gov.flagged) {
      const house = HOUSES[gov.house];
      const houseWeight = house ? house.members.length / total : 0.05;
      governancePenalty += houseWeight * 0.3; // 30% of house-weight per sick governor
    }
  }

  // Portal penalty: imbalanced portals affect the whole system
  const portalPenalty =
    (aggregates.statuses.Balanced === 0 ? 0.1 : 0) + // catastrophic
    (flags.filter(f => f.type === 'portal-as-transient').length * 0.03);

  // Sick rebalancer penalty
  const sickRebalancerPenalty = flags.filter(f => f.type === 'sick-rebalancer').length * 0.02;

  // Self-home bonus: grounded positions are stabilizing
  const selfHomeBonus = flags.filter(f => f.type === 'self-home').length * 0.01;

  // Wheel/World field imbalance penalty: large differential = structural asymmetry
  const fieldImbalancePenalty = wheelWorldFields
    ? Math.abs(wheelWorldFields.differential) * 0.05
    : 0;

  // Corrective vector bonus/penalty: configuration moving toward health helps
  const vectorAdjustment = correctiveVector
    ? correctiveVector.vector * 0.05
    : 0;

  const raw = balancedRatio
    - governancePenalty
    - portalPenalty
    - sickRebalancerPenalty
    + selfHomeBonus
    - fieldImbalancePenalty
    + vectorAdjustment;
  const score = Math.max(0, Math.min(1, raw));

  return {
    score: Math.round(score * 100) / 100,
    balancedRatio: Math.round(balancedRatio * 100) / 100,
    governancePenalty: Math.round(governancePenalty * 100) / 100,
    portalPenalty: Math.round(portalPenalty * 100) / 100,
    sickRebalancerPenalty: Math.round(sickRebalancerPenalty * 100) / 100,
    selfHomeBonus: Math.round(selfHomeBonus * 100) / 100,
    fieldImbalancePenalty: Math.round(fieldImbalancePenalty * 100) / 100,
    vectorAdjustment: Math.round(vectorAdjustment * 100) / 100
  };
}
...
export function triageReading(analysis, drawMap) {
  ...
  const severity = distortionCount === 0 ? 'healthy'
    : distortionCount <= 3 ? 'minor'
    : distortionCount <= 7 ? 'moderate'
    : distortionCount <= 14 ? 'significant'
    : 'systemic';
...
function buildProcessOverview(drawMap, positions) {
  ...
  for (const proc of processes) {
    if (!byHouse[proc.house]) byHouse[proc.house] = { healthy: 0, distorted: 0, processes: [] };
