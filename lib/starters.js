...
// Every sentence a house holds, flat: the four everyday starters and the three deep ones.
// "Let one be chosen for me" draws from this whole pool, so a daily reading can land anywhere
// from "I'm working on my health" to "Why is the universe so big?".
export const allStartersFor = (id) => {
  const set = STARTERS[id];
  if (!set) return [];
  return [set.working, set.want, set.unsure, set.bothering, ...(set.bigger || [])].filter(Boolean);
};
