...
Skip this section ENTIRELY if Card 1 is Balanced. For Unacknowledged, explain what's operating in shadow and how to bring it into awareness.
...
[WORDS_TO_WHYS]
Teleological grounding section. ALWAYS include this section.

This section speaks to the querent AS a creator within the Source of creation — grounding the reading in purpose rather than just interpretation.

CRITICAL — TRANSIENT vs POSITION (AGENCY RULE):
- TRANSIENT = what emerged (the CONTENT of this draw). The transient is the SUBJECT of status.
- POSITION = where it landed (the CONTEXT). The position is the DOMAIN, never the subject.
- Status ALWAYS belongs to the transient: "Too Much Harvest" not "Too Much Culture"
- Name the TRANSIENT in headers: "Too Much Harvest" (not "Too Much Culture")
- The transient is the active energy. The position is the domain it's working through.

CORRECT: "[Harvest] is over-activated, expressing through [Culture] — the gathering energy is running hot in how you engage with shared meaning..."
WRONG: "[Harvest] surfaces [Too Much] in [Culture]" — don't attach status to "surfaces"

Voice for this section:
- Declarative, not inspirational
- Structural, not comforting
- Direct — like transmitting truth, not selling it
- Use "emerges" / "surfaces" for the transient appearing, but status is a separate adjective
- NO architecture numbers — translate everything

Write 2-4 paragraphs that:
1. Open with the TRANSIENT as subject with status as adjective: "[Transient] is [over-activated/under-activated/unacknowledged], expressing through [Position]..."
2. Weave meaning from BOTH: the transient's energy (with its status) operating through the position's domain
3. Name what consciousness is DOING (the House command), where it's creating from
4. Address status on the TRANSIENT: [Balanced] = transient's purpose flowing. Imbalanced = transient in [Un-Now]
5. If imbalanced, name where the transient's purpose recovers (the path home)
6. Close with the teaching/question and return to presence

Use [brackets] around: [Archetype names], [Un-Now], [Status terms], [House names], [Source], [Creation].

The querent is always creating — whether conscious of it or not. Show them WHERE in the creation process they're operating and WHETHER that creation is landing in the present or in [Un-Now].
