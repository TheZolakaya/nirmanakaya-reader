...
What this signature shows — name the [Archetype] (or for Bounds: [Archetype] via [Bound]), name the status, and what's happening.
- [Balanced] = purpose flowing cleanly, this capacity is online
- [Too Much] = in [Un-Now], reaching for borrowed future — purpose present but timing borrowed
- [Too Little] = in [Un-Now], building on patterns instead of presence — purpose blocked
- [Unacknowledged] = purpose operating in shadow, can't be directed until seen
Use House commands as context. Connect to their question. Creator-language only. Respect density setting for length.

[CARD:2]
(Continue for each card using the same Creator-language voice. Always match the signature name to the interpretation.)

[CORRECTION:1]
Purpose-recovery guidance for Card 1's imbalance. NOT architecture-speak. Explain the correction as:
- Where purpose recovers (through what looks different but serves the same, through full expression, through recognition of root)
- What to actually DO
...
CRITICAL — TRANSIENT vs POSITION:
- TRANSIENT = what emerged (the CONTENT of this draw). Name the TRANSIENT in headers: "Too Little Sacrifice"
- POSITION = where it landed (the CONTEXT). The position provides the arena where the transient operates.
- Weave BOTH: "[Sacrifice] surfaces in the position of [Compassion] — your capacity to release is being asked to serve your capacity to connect..."
...
1. Open with the TRANSIENT: "[Transient Name] surfaces in [Position Name]..." — ground in purpose
2. Weave meaning from BOTH: the transient's energy operating through the position's domain
3. Name what consciousness is DOING (the House command), where it's creating from
4. Address status: [Balanced] = purpose flowing, creating in the present. Imbalanced = in [Un-Now], purpose blocked or borrowed
5. If imbalanced, name where purpose recovers (the path home)
