 ...
 * @param {object[]|null} narrativeReadings - [{narrative_summary, hashtags, created_at, topic}] recent readings with summaries
 * @returns {string} Context block text, or '' if insufficient data
 */
export function buildUserContextBlock(stats, currentDraws, recentReadings, topicData, profile, personalFacts, narrativeReadings) {
  ...
  // ── Journey Thread (narrative summaries from recent readings) ──
  if (narrativeReadings && narrativeReadings.length > 0) {
    const withSummary = narrativeReadings.filter(r => r.narrative_summary && r.narrative_summary !== '__no_content__');
    if (withSummary.length > 0) {
      const now = new Date();
      const todayStart = new Date(now.getFullYear(), now.getMonth(), now.getDate());
      const weekAgo = new Date(now); weekAgo.setDate(weekAgo.getDate() - 7);

      const todayNarratives = withSummary.filter(r => new Date(r.created_at) >= todayStart);
      const weekNarratives = withSummary.filter(r => new Date(r.created_at) >= weekAgo && new Date(r.created_at) < todayStart);
      const olderNarratives = withSummary.filter(r => new Date(r.created_at) < weekAgo);

      // Themes, ordered by RECENCY, shown without counts. Frequency was being read as importance:
      // a person who has asked about one thing many times got that thing woven into every
      // reading whatever they asked today (the founder, 2026-09-16). Today's mention weighs
      // three, this week's two, older one — and the count itself is never shown.
      const tagScore = {};
      for (const r of withSummary) {
        const when = new Date(r.created_at);
        const w = when >= todayStart ? 3 : when >= weekAgo ? 2 : 1;
        for (const tag of (r.hashtags || [])) tagScore[tag] = (tagScore[tag] || 0) + w;
      }
      ...
      lines.push('');
      lines.push('=== JOURNEY THREAD ===');

      if (todayNarratives.length > 0) {
        const todaySummaries = todayNarratives.slice(0, 3).map(r => r.narrative_summary).join(' ');
        lines.push(`Today: ${todaySummaries}`);
      }

      if (weekNarratives.length > 0) {
        const weekSummaries = weekNarratives.slice(0, 3).map(r => r.narrative_summary).join(' ');
        lines.push(`This week: ${weekSummaries}`);
      }

      if (olderNarratives.length > 0) {
        const olderSummaries = olderNarratives.slice(0, 2).map(r => r.narrative_summary).join(' ');
        lines.push(`Earlier: ${olderSummaries}`);
      }

      if (topTags.length > 0) {
        lines.push(`Themes they have brought before (most recent first): ${topTags.join(', ')}.`);
      }

      lines.push('THIS IS BACKGROUND, NOT SUBJECT. The question asked today leads. How often a theme was brought before is how often it was asked about, not how much it matters now — do not steer the reading toward a past theme, and do not open with one unless the person brought it up themselves. Draw on this thread only where it bears directly on what they asked today; otherwise leave it in the room unmentioned.');
      lines.push('=== END JOURNEY THREAD ===');
    }
  }

  const result = lines.join('\n');

  // Hard cap: if over ~600 tokens (rough 4 chars/token), truncate less critical sections
  if (result.length > 2400) {
    return lines
      .filter(l => !l.startsWith('Houses:') && !l.startsWith('Status:') && !l.startsWith('Earlier:'))
      .join('\n');
  }
