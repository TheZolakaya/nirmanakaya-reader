...
import { presetById, drawFor, drawLabel, SECTIONS, HOSTILE, parseJson } from './presets.js';
...
// THE SECTIONS A BATCH RUNS: all six doors, or — for a HOSTILE batch — the opening only (the rail
// under test lives in the opening; the floors would only repeat the verdict).
export const sectionsOf = (b) => (b?.hostile ? SECTIONS.filter((s) => s.id === 'opening') : SECTIONS);

export function summary(b) {
  const total = b.n * sectionsOf(b).length, done = b.runs.reduce((n, r) => n + Object.keys(r.sections || {}).length, 0);
...
export function createBatch({ n = 10, lane = 'model', models = [], model = 'sonnet', variants = [], author = null, hostile = false }) {
  let lanes;
  if (lane === 'prompt') {
  ...
  let runs;
  if (hostile) {
    // THE HOSTILE BATCH (.471): the twelve beinghood questions with their FIXED draws, so every model
    // meets the same field. The preset rides on the run so the lint knows to look for the hedge.
    runs = HOSTILE.map(([q], i) => {
      const preset = presetById(`hostile:${i + 1}`);
      const draw = drawFor(preset, null);
      return { i: i + 1, question: q, door: 'hostile', kind: 'hostile', preset: preset.id, draw, drawLabel: drawLabel(draw), sections: {}, sharedOpening: null, sharedOpeningCost: 0 };
    });
    n = runs.length;
  } else {
  ...
  const b = { id: shortId(), code: code(), created: new Date().toISOString(), version: VERSION, author, lane, n, lanes, hostile: !!hostile, status: 'queued', current: null, runs };
  return writeBatch(b);
}
...
export async function runBatch(id) {
  ...
  try {
    for (const run of b.runs) {
      if (Object.keys(run.sections).length === sectionsOf(b).length) continue; // resumed batch: already done
      for (const sec of sectionsOf(b)) {
        if (run.sections[sec.id]) continue;
        b.current = { run: run.i, section: sec.id }; writeBatch(b);
        const preset = presetById(run.preset || sec.preset); // a hostile run carries its own preset (the lint looks for the hedge)
...
export function blindView(b, judgedSections = {}) {
  const runs = b.runs.map((run) => {
    const done = sectionsOf(b).every((s) => judgedSections[`${run.i}:${s.id}`]);
    const sections = {};
    for (const s of sectionsOf(b)) {
  ...
  return { id: b.id, version: b.version, lane: b.lane, n: b.n, hostile: !!b.hostile, status: b.status, laneCount: b.lanes.length, runs };
}
...
export function exportMarkdown(b) {
  ...
  L.push(`# BAKE-OFF BATCH ${b.id} — blind, for a seat to judge`, '', `**Reader version:** ${b.version} · **lane:** ${b.lane} · **runs:** ${b.n} · **columns per section:** ${b.lanes.length} · **created:** ${b.created}`, '',
    '> You are judging blind. Each section shows the same prompt answered by different columns, lettered in a random order that differs per section. For every section, name the letter you prefer — or "tie". Add "!" for a strong preference. Return your picks as lines like:', '>', '> `run 3, meaning: B`  ·  `run 3, moon: A!`  ·  `run 4, opening: tie`', '>', `> Sections: ${sectionsOf(b).map((s) => s.id).join(' · ')}${b.hostile ? ' — a HOSTILE batch: the twelve beinghood questions, the opening only (the rail under test lives there)' : ''}. The pick is the reading; cost is not shown here on purpose. Judge the rails too: the Plain voice forbids the map\'s words on glass; the opening must not end with its own question; on a beinghood question the assistant\'s hedge is a failure.`, '');
  for (const run of b.runs) {
