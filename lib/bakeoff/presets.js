...
import { VOICES, EZ_RULES, BRAZIER_HARD_RULE, BRAZIER_RULES, DRAGON_STANDARD, dragonBlock, doSomethingBlock, fmtDrawForVoice } from '../ezPrompts.js';
...
export const PRESETS = [
  ...
  { id: 'floor:mechanism', label: 'Floor: the mechanism', kind: 'floor', floor: 'mechanism' },
  { id: 'dragon', label: 'Face the dragon', kind: 'dragon' },
  { id: 'step', label: 'One small step', kind: 'step' },
...
// THE SIX SECTIONS OF A BATCH RUN (addendum 4): every door pushed, in the page's own order.
export const SECTIONS = [
  { id: 'opening', label: 'the opening', preset: 'ez-opening' },
  { id: 'meaning', label: 'the meaning', preset: 'floor:meaning' },
  { id: 'moon', label: 'the moon', preset: 'floor:moon' },
  { id: 'mechanism', label: 'the mechanism', preset: 'floor:mechanism' },
  { id: 'dragon', label: 'the dragon', preset: 'dragon' },
  { id: 'step', label: 'one small step', preset: 'step' },
];
...
// ONE SMALL STEP — page.js fetchStep(): the original draw, the discourse + the asker's act line
// (the opening's own "act" field when it wrote one, else the page's default line), the card in
// play with its record and teleology, the do-something block; the EZ system; cap 500.
export function buildStep({ question, draw, opening, over }) {
  const q = sanitizeForAPI(String(question || '').trim());
  const k = buildKernel(draw, DEFS);
  const draws = [draw];
  const drawText = fmtDrawForVoice('plain', draws, 'discover', spreadKeyFor(1), false, null, null, null);
  const line = (typeof opening?.act === 'string' && opening.act.trim()) || `What is one small real thing I can do about this in the next minute?`;
  const asked = `${discourseOf(q, opening)}\n\nASKER (asks for one small thing to do): "${line}"`;
  let tele = ''; try { tele = buildReadingTeleologicalPrompt([draw]); } catch {}
  const message = `QUESTION: "${q}"\n\nTHE ORIGINAL DRAW (unchanged):\n${drawText}\n\nTHE DISCOURSE SO FAR, in order:\n${asked}\n\nTHE CARD IN PLAY:\n${drawRecord(draw, DEFS)}${tele ? `\n\n${tele}` : ''}${doSomethingBlock(k)}`;
  return { system: blocks(over).ezSystem, message, maxTokens: 500 };
}
...
export function buildPrompt(preset, ctx) {
  ...
  if (preset.kind === 'step') return buildStep(ctx);
  throw new Error('unknown preset kind ' + preset.kind);
}

// The page's hard word limits for the floors (fetchFloor LIMIT, .469): a render over 115% of
// its limit is rewritten once, and the shorter render kept. The bench mirrors it in lib/bakeoff/run.js.
export const FLOOR_LIMIT = { meaning: 250, moon: 250, mechanism: 300 };
