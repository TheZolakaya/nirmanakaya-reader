...
export async function callModel({ modelKey, system, message, maxTokens }) {
  ...
  if (m.provider === 'deepseek') body.thinking = m.thinking ? { type: 'enabled', budget_tokens: 4096 } : { type: 'disabled' };
  ...
  try {
    ...
    if (data?.error && m.provider === 'deepseek' && /thinking/i.test(JSON.stringify(data.error))) {
