# Canonical 78 Corrections Reference

**Created:** January 11, 2026
**Status:** CANONICAL - This is the authoritative source for ALL 78 signature corrections and growth opportunities
**Authority:** Chris's direct specification

---

## THE LOGIC

### For Imbalanced States (Too Much, Too Little, Unacknowledged)
Archetypes correct to their duality partner:
- **Too Much → Diagonal partner**
- **Too Little → Vertical partner**
- **Unacknowledged → Reduction partner**

For Bounds and Agents: Follow the associated Archetype's correction path with **polarity flip**.

### For Balanced State (Growth Opportunity)
Tune isn't a destination — it's a launchpad. When balanced, the archetype points to its **growth partner**:

- **12 Off-Diagonal Archetypes → Transpose Partner** (lateral growth through reflection)
- **4 Diagonal Archetypes → Polarity Anchor** (vertical growth through elemental opposition)

For Bounds and Agents: Follow the associated Archetype's growth partner with **polarity flip**.

### Polarity Reference
- **Inner:** Ace, 2, 3, 4, 5 / Page, Knight
- **Outer:** 6, 7, 8, 9, 10 / Queen, King

---

## PART I: ARCHETYPE CORRECTION PAIRS

### Diagonal Pairs (Too Much correction)
| Archetype | Traditional | Diagonal Partner | Traditional |
|-----------|-------------|------------------|-------------|
| Potential (0) | Fool | Actualization (19) | Sun |
| Will (1) | Magician | Recognition (20) | Judgement |
| Wisdom (2) | High Priestess | Inspiration (17) | Star |
| Nurturing (3) | Empress | Imagination (18) | Moon |
| Order (4) | Emperor | Abstraction (15) | Devil |
| Culture (5) | Hierophant | Breakthrough (16) | Tower |
| Compassion (6) | Lovers | Change (13) | Death |
| Drive (7) | Chariot | Tune (14) | Temperance |
| Fortitude (8) | Strength | Equity (11) | Justice |
| Discipline (9) | Hermit | Sacrifice (12) | Hanged Man |
| Equity (11) | Justice | Fortitude (8) | Strength |
| Sacrifice (12) | Hanged Man | Discipline (9) | Hermit |
| Change (13) | Death | Compassion (6) | Lovers |
| Tune (14) | Temperance | Drive (7) | Chariot |
| Abstraction (15) | Devil | Order (4) | Emperor |
| Breakthrough (16) | Tower | Culture (5) | Hierophant |
| Inspiration (17) | Star | Wisdom (2) | High Priestess |
| Imagination (18) | Moon | Nurturing (3) | Empress |
| Actualization (19) | Sun | Potential (0) | Fool |
| Recognition (20) | Judgement | Will (1) | Magician |

### Vertical Pairs (Too Little correction)
| Archetype | Traditional | Vertical Partner | Traditional |
|-----------|-------------|------------------|-------------|
| Potential (0) | Fool | Recognition (20) | Judgement |
| Will (1) | Magician | Actualization (19) | Sun |
| Wisdom (2) | High Priestess | Imagination (18) | Moon |
| Nurturing (3) | Empress | Inspiration (17) | Star |
| Order (4) | Emperor | Breakthrough (16) | Tower |
| Culture (5) | Hierophant | Abstraction (15) | Devil |
| Compassion (6) | Lovers | Tune (14) | Temperance |
| Drive (7) | Chariot | Change (13) | Death |
| Fortitude (8) | Strength | Sacrifice (12) | Hanged Man |
| Discipline (9) | Hermit | Equity (11) | Justice |
| Equity (11) | Justice | Discipline (9) | Hermit |
| Sacrifice (12) | Hanged Man | Fortitude (8) | Strength |
| Change (13) | Death | Drive (7) | Chariot |
| Tune (14) | Temperance | Compassion (6) | Lovers |
| Abstraction (15) | Devil | Culture (5) | Hierophant |
| Breakthrough (16) | Tower | Order (4) | Emperor |
| Inspiration (17) | Star | Nurturing (3) | Empress |
| Imagination (18) | Moon | Wisdom (2) | High Priestess |
| Actualization (19) | Sun | Will (1) | Magician |
| Recognition (20) | Judgement | Potential (0) | Fool |

### Reduction Pairs (Unacknowledged correction)
| Archetype | Traditional | Reduction Partner | Traditional |
|-----------|-------------|-------------------|-------------|
| Potential (0) | Fool | — | No reduction pair |
| Will (1) | Magician | — | No reduction pair |
| Wisdom (2) | High Priestess | Equity (11) | Justice |
| Nurturing (3) | Empress | Sacrifice (12) | Hanged Man |
| Order (4) | Emperor | Change (13) | Death |
| Culture (5) | Hierophant | Tune (14) | Temperance |
| Compassion (6) | Lovers | Abstraction (15) | Devil |
| Drive (7) | Chariot | Breakthrough (16) | Tower |
| Fortitude (8) | Strength | Inspiration (17) | Star |
| Discipline (9) | Hermit | Imagination (18) | Moon |
| Source (10) | Wheel | — | No reduction pair |
| Equity (11) | Justice | Wisdom (2) | High Priestess |
| Sacrifice (12) | Hanged Man | Nurturing (3) | Empress |
| Change (13) | Death | Order (4) | Emperor |
| Tune (14) | Temperance | Culture (5) | Hierophant |
| Abstraction (15) | Devil | Compassion (6) | Lovers |
| Breakthrough (16) | Tower | Drive (7) | Chariot |
| Inspiration (17) | Star | Fortitude (8) | Strength |
| Imagination (18) | Moon | Discipline (9) | Hermit |
| Actualization (19) | Sun | — | No reduction pair |
| Recognition (20) | Judgement | — | No reduction pair |
| Creation (21) | World | — | No reduction pair |

---

## PART II: GROWTH PAIRS (Balanced State)

### Transpose Pairs — 12 Off-Diagonal Archetypes (Lateral Growth)

| Archetype | Traditional | Growth Partner | Traditional | Dynamic |
|-----------|-------------|----------------|-------------|---------|
| Wisdom (2) | High Priestess | Order (4) | Emperor | Reception → Structure |
| Nurturing (3) | Empress | Sacrifice (12) | Hanged Man | Cultivation → Surrender |
| Order (4) | Emperor | Wisdom (2) | High Priestess | Structure → Reception |
| Culture (5) | Hierophant | Tune (14) | Temperance | Transmission → Integration |
| Drive (7) | Chariot | Imagination (18) | Moon | Momentum → Vision |
| Discipline (9) | Hermit | Breakthrough (16) | Tower | Method → Clearing |
| Equity (11) | Justice | Change (13) | Death | Fairness → Transformation |
| Sacrifice (12) | Hanged Man | Nurturing (3) | Empress | Surrender → Cultivation |
| Change (13) | Death | Equity (11) | Justice | Transformation → Fairness |
| Tune (14) | Temperance | Culture (5) | Hierophant | Integration → Transmission |
| Breakthrough (16) | Tower | Discipline (9) | Hermit | Clearing → Method |
| Imagination (18) | Moon | Drive (7) | Chariot | Vision → Momentum |

### Polarity Pairs — 4 Diagonal Archetypes (Vertical Growth)

| Archetype | Traditional | Growth Partner | Traditional | Axis | Dynamic |
|-----------|-------------|----------------|-------------|------|---------|
| Compassion (6) | Lovers | Abstraction (15) | Devil | Water ↔ Air | Feeling → Logic |
| Fortitude (8) | Strength | Inspiration (17) | Star | Earth ↔ Fire | Embodiment → Vision |
| Abstraction (15) | Devil | Compassion (6) | Lovers | Air ↔ Water | Logic → Feeling |
| Inspiration (17) | Star | Fortitude (8) | Strength | Fire ↔ Earth | Vision → Embodiment |

### Mathematical Validation

**Transpose Pairs Sum Reductions:**
| Pair | Sum | Reduction | Force Generated |
|------|-----|-----------|-----------------|
| Order (4) + Wisdom (2) | 6 | 6 | Connection |
| Drive (7) + Imagination (18) | 25 | 7 | Momentum |
| Sacrifice (12) + Nurturing (3) | 15 | 6 | Connection |
| Tune (14) + Culture (5) | 19 | 1 | Will |
| Discipline (9) + Breakthrough (16) | 25 | 7 | Momentum |
| Equity (11) + Change (13) | 24 | 6 | Connection |

**Polarity Pairs Mod-9 Twinning:**
| Pair | Mod-9 |
|------|-------|
| Inspiration (17) ↔ Fortitude (8) | Both = 8 |
| Abstraction (15) ↔ Compassion (6) | Both = 6 |

---

## PART III: ARCHETYPE (MAJOR) COMPLETE REFERENCE

| Major | Too Much → | Too Little → | Unacknowledged → | **Balanced → (Growth)** |
|-------|------------|--------------|------------------|-------------------------|
| **Fool (0)** | Sun | Judgement | **Fool** | **Fool** |
| **Magician (1)** | Judgement | Sun | **Magician** | **Magician** |
| **High Priestess (2)** | Star | Moon | Justice | **Emperor** |
| **Empress (3)** | Moon | Star | Hanged Man | **Hanged Man** |
| **Emperor (4)** | Devil | Tower | Death | **High Priestess** |
| **Hierophant (5)** | Tower | Devil | Temperance | **Temperance** |
| **Lovers (6)** | Death | Temperance | Devil | **Devil** |
| **Chariot (7)** | Temperance | Death | Tower | **Moon** |
| **Strength (8)** | Justice | Hanged Man | Star | **Star** |
| **Hermit (9)** | Hanged Man | Justice | Moon | **Tower** |
| **Wheel (10)** | **Wheel** | **Wheel** | **Wheel** | **Wheel** |
| **Justice (11)** | Strength | Hermit | High Priestess | **Death** |
| **Hanged Man (12)** | Hermit | Strength | Empress | **Empress** |
| **Death (13)** | Lovers | Chariot | Emperor | **Justice** |
| **Temperance (14)** | Chariot | Lovers | Hierophant | **Hierophant** |
| **Devil (15)** | Emperor | Hierophant | Lovers | **Lovers** |
| **Tower (16)** | Hierophant | Emperor | Chariot | **Hermit** |
| **Star (17)** | High Priestess | Empress | Strength | **Strength** |
| **Moon (18)** | Empress | High Priestess | Hermit | **Chariot** |
| **Sun (19)** | Fool | Magician | **Sun** | **Sun** |
| **Judgement (20)** | Magician | Fool | **Judgement** | **Judgement** |
| **World (21)** | **World** | **World** | **World** | **World** |

---

## PART IV: ARCHETYPE → BOUNDS/AGENTS REFERENCE

| Archetype | Traditional | Inner Bound | Outer Bound | Inner Agent | Outer Agent |
|-----------|-------------|-------------|-------------|-------------|-------------|
| Potential (0) | Fool | Ace of Wands | 10 of Wands | — | — |
| Will (1) | Magician | Ace of Pentacles | 10 of Pentacles | — | — |
| Wisdom (2) | High Priestess | 2 of Swords | 9 of Swords | Page of Swords | — |
| Nurturing (3) | Empress | 2 of Pentacles | 9 of Pentacles | Page of Pentacles | — |
| Order (4) | Emperor | 3 of Wands | 8 of Wands | Knight of Wands | — |
| Culture (5) | Hierophant | 3 of Cups | 8 of Cups | Knight of Cups | — |
| Compassion (6) | Lovers | 4 of Cups | 7 of Cups | — | Queen of Cups |
| Drive (7) | Chariot | 4 of Wands | 7 of Wands | — | Queen of Wands |
| Fortitude (8) | Strength | 5 of Pentacles | 6 of Pentacles | — | King of Pentacles |
| Discipline (9) | Hermit | 5 of Swords | 6 of Swords | — | King of Swords |
| Equity (11) | Justice | 5 of Cups | 6 of Cups | — | King of Cups |
| Sacrifice (12) | Hanged Man | 5 of Wands | 6 of Wands | — | King of Wands |
| Change (13) | Death | 4 of Pentacles | 7 of Pentacles | — | Queen of Pentacles |
| Tune (14) | Temperance | 4 of Swords | 7 of Swords | — | Queen of Swords |
| Abstraction (15) | Devil | 3 of Swords | 8 of Swords | Knight of Swords | — |
| Breakthrough (16) | Tower | 3 of Pentacles | 8 of Pentacles | Knight of Pentacles | — |
| Inspiration (17) | Star | 2 of Wands | 9 of Wands | Page of Wands | — |
| Imagination (18) | Moon | 2 of Cups | 9 of Cups | Page of Cups | — |
| Actualization (19) | Sun | Ace of Swords | 10 of Swords | — | — |
| Recognition (20) | Judgement | Ace of Cups | 10 of Cups | — | — |

---

## PART V: BOUND (MINOR) COMPLETE REFERENCE

### WANDS

| Bound | Too Much → | Too Little → | Unacknowledged → | **Balanced → (Growth)** |
|-------|------------|--------------|------------------|-------------------------|
| **Ace of Wands** | 10 of Swords | 10 of Cups | **Ace of Wands** | **Ace of Wands** |
| **2 of Wands** | 9 of Swords | 9 of Pentacles | 6 of Pentacles | **6 of Pentacles** |
| **3 of Wands** | 8 of Swords | 8 of Pentacles | 7 of Pentacles | **8 of Swords** |
| **4 of Wands** | 7 of Swords | 7 of Pentacles | 8 of Pentacles | **7 of Cups** |
| **5 of Wands** | 6 of Swords | 6 of Pentacles | 9 of Pentacles | **9 of Pentacles** |
| **6 of Wands** | 5 of Swords | 5 of Pentacles | 2 of Pentacles | **2 of Pentacles** |
| **7 of Wands** | 4 of Swords | 4 of Pentacles | 3 of Pentacles | **4 of Cups** |
| **8 of Wands** | 3 of Swords | 3 of Pentacles | 4 of Pentacles | **3 of Swords** |
| **9 of Wands** | 2 of Swords | 2 of Pentacles | 5 of Pentacles | **5 of Pentacles** |
| **10 of Wands** | Ace of Swords | Ace of Cups | **10 of Wands** | **10 of Wands** |

### SWORDS

| Bound | Too Much → | Too Little → | Unacknowledged → | **Balanced → (Growth)** |
|-------|------------|--------------|------------------|-------------------------|
| **Ace of Swords** | 10 of Wands | 10 of Pentacles | **Ace of Swords** | **Ace of Swords** |
| **2 of Swords** | 9 of Wands | 9 of Cups | 6 of Cups | **8 of Wands** |
| **3 of Swords** | 8 of Wands | 8 of Cups | 7 of Cups | **7 of Cups** |
| **4 of Swords** | 7 of Wands | 7 of Cups | 8 of Cups | **8 of Cups** |
| **5 of Swords** | 6 of Wands | 6 of Cups | 9 of Cups | **8 of Pentacles** |
| **6 of Swords** | 5 of Wands | 5 of Cups | 2 of Cups | **3 of Pentacles** |
| **7 of Swords** | 4 of Wands | 4 of Cups | 3 of Cups | **3 of Cups** |
| **8 of Swords** | 3 of Wands | 3 of Cups | 4 of Cups | **4 of Cups** |
| **9 of Swords** | 2 of Wands | 2 of Cups | 5 of Cups | **3 of Wands** |
| **10 of Swords** | Ace of Wands | Ace of Pentacles | **10 of Swords** | **10 of Swords** |

### CUPS

| Bound | Too Much → | Too Little → | Unacknowledged → | **Balanced → (Growth)** |
|-------|------------|--------------|------------------|-------------------------|
| **Ace of Cups** | 10 of Pentacles | 10 of Wands | **Ace of Cups** | **Ace of Cups** |
| **2 of Cups** | 9 of Pentacles | 9 of Swords | 6 of Swords | **7 of Wands** |
| **3 of Cups** | 8 of Pentacles | 8 of Swords | 7 of Swords | **7 of Swords** |
| **4 of Cups** | 7 of Pentacles | 7 of Swords | 8 of Swords | **8 of Swords** |
| **5 of Cups** | 6 of Pentacles | 6 of Swords | 9 of Swords | **7 of Pentacles** |
| **6 of Cups** | 5 of Pentacles | 5 of Swords | 2 of Swords | **4 of Pentacles** |
| **7 of Cups** | 4 of Pentacles | 4 of Swords | 3 of Swords | **3 of Swords** |
| **8 of Cups** | 3 of Pentacles | 3 of Swords | 4 of Swords | **4 of Swords** |
| **9 of Cups** | 2 of Pentacles | 2 of Swords | 5 of Swords | **4 of Wands** |
| **10 of Cups** | Ace of Pentacles | Ace of Wands | **10 of Cups** | **10 of Cups** |

### PENTACLES

| Bound | Too Much → | Too Little → | Unacknowledged → | **Balanced → (Growth)** |
|-------|------------|--------------|------------------|-------------------------|
| **Ace of Pentacles** | 10 of Cups | 10 of Swords | **Ace of Pentacles** | **Ace of Pentacles** |
| **2 of Pentacles** | 9 of Cups | 9 of Wands | 6 of Wands | **6 of Wands** |
| **3 of Pentacles** | 8 of Cups | 8 of Wands | 7 of Wands | **6 of Swords** |
| **4 of Pentacles** | 7 of Cups | 7 of Wands | 8 of Wands | **6 of Cups** |
| **5 of Pentacles** | 6 of Cups | 6 of Wands | 9 of Wands | **9 of Wands** |
| **6 of Pentacles** | 5 of Cups | 5 of Wands | 2 of Wands | **2 of Wands** |
| **7 of Pentacles** | 4 of Cups | 4 of Wands | 3 of Wands | **5 of Cups** |
| **8 of Pentacles** | 3 of Cups | 3 of Wands | 4 of Wands | **5 of Swords** |
| **9 of Pentacles** | 2 of Cups | 2 of Wands | 5 of Wands | **5 of Wands** |
| **10 of Pentacles** | Ace of Cups | Ace of Swords | **10 of Pentacles** | **10 of Pentacles** |

---

## PART VI: AGENT (ROYAL) COMPLETE REFERENCE

### Agent → Archetype Reference
| Agent | Traditional | Archetype | Polarity |
|-------|-------------|-----------|----------|
| Page of Wands | Initiate of Intent | Inspiration (17) | Inner |
| Knight of Wands | Catalyst of Intent | Order (4) | Inner |
| Queen of Wands | Steward of Intent | Drive (7) | Outer |
| King of Wands | Executor of Intent | Sacrifice (12) | Outer |
| Page of Swords | Initiate of Cognition | Wisdom (2) | Inner |
| Knight of Swords | Catalyst of Cognition | Abstraction (15) | Inner |
| Queen of Swords | Steward of Cognition | Tune (14) | Outer |
| King of Swords | Executor of Cognition | Discipline (9) | Outer |
| Page of Cups | Initiate of Resonance | Imagination (18) | Inner |
| Knight of Cups | Catalyst of Resonance | Culture (5) | Inner |
| Queen of Cups | Steward of Resonance | Compassion (6) | Outer |
| King of Cups | Executor of Resonance | Equity (11) | Outer |
| Page of Pentacles | Initiate of Structure | Nurturing (3) | Inner |
| Knight of Pentacles | Catalyst of Structure | Breakthrough (16) | Inner |
| Queen of Pentacles | Steward of Structure | Change (13) | Outer |
| King of Pentacles | Executor of Structure | Fortitude (8) | Outer |

### Agents Complete Reference

| Agent | Too Much → | Too Little → | Unacknowledged → | **Balanced → (Growth)** |
|-------|------------|--------------|------------------|-------------------------|
| **Page of Wands** | King of Swords | King of Pentacles | King of Pentacles | **King of Pentacles** |
| **Knight of Wands** | Queen of Swords | Queen of Pentacles | Queen of Pentacles | **Queen of Swords** |
| **Queen of Wands** | Knight of Swords | Knight of Pentacles | Knight of Pentacles | **Knight of Cups** |
| **King of Wands** | Page of Swords | Page of Pentacles | Page of Pentacles | **Page of Pentacles** |
| **Page of Swords** | King of Wands | King of Cups | King of Cups | **Queen of Wands** |
| **Knight of Swords** | Queen of Wands | Queen of Cups | Queen of Cups | **Queen of Cups** |
| **Queen of Swords** | Knight of Wands | Knight of Cups | Knight of Cups | **Knight of Cups** |
| **King of Swords** | Page of Wands | Page of Cups | Page of Cups | **Page of Pentacles** |
| **Page of Cups** | King of Pentacles | King of Swords | King of Swords | **Queen of Wands** |
| **Knight of Cups** | Queen of Pentacles | Queen of Swords | Queen of Swords | **Queen of Swords** |
| **Queen of Cups** | Knight of Pentacles | Knight of Swords | Knight of Swords | **Knight of Swords** |
| **King of Cups** | Page of Pentacles | Page of Swords | Page of Swords | **Page of Swords** |
| **Page of Pentacles** | King of Cups | King of Wands | King of Wands | **King of Wands** |
| **Knight of Pentacles** | Queen of Cups | Queen of Wands | Queen of Wands | **Queen of Swords** |
| **Queen of Pentacles** | Knight of Cups | Knight of Wands | Knight of Wands | **Knight of Cups** |
| **King of Pentacles** | Page of Cups | Page of Wands | Page of Wands | **Page of Wands** |

---

## PART VII: DEVELOPMENTAL GUIDANCE FOR BALANCED STATES

### Transpose Pair Guidance Examples

**Order (Emperor) Balanced → Wisdom (High Priestess):**
"Your structure is sound — your systems work. Now, learn to receive. Structure without receptivity becomes rigidity."

**Wisdom (High Priestess) Balanced → Order (Emperor):**
"Your receptivity is open — you see what is. Now, give it form. Reception without structure remains formless."

**Drive (Chariot) Balanced → Imagination (Moon):**
"Your momentum is unstoppable — you charge forward. Now, let the dream inform the direction. Momentum without vision is just velocity."

**Imagination (Moon) Balanced → Drive (Chariot):**
"Your vision is crystalline — you see what could be. Now, move toward it. Vision without momentum remains fantasy."

**Sacrifice (Hanged Man) Balanced → Nurturing (Empress):**
"You've learned to let go — surrender is natural. Now, learn to grow. Surrender without cultivation is emptiness."

**Nurturing (Empress) Balanced → Sacrifice (Hanged Man):**
"Your garden flourishes — you know how to tend. Now, learn what to prune. Cultivation without surrender becomes overgrowth."

### Polarity Pair Guidance Examples

**Inspiration (Star) Balanced → Fortitude (Strength):**
"Your vision is blindingly bright. Now, give it weight. Embody it. If it can't lift a stone, it isn't real."

**Fortitude (Strength) Balanced → Inspiration (Star):**
"Your structure is unbreakable. Now, let light through it. Inspire it. If it doesn't serve a higher vision, it's just a cage."

**Abstraction (Devil) Balanced → Compassion (Lovers):**
"The pattern is perfect. But does it bleed? Feel the geometry. Logic without Compassion is cruelty."

**Compassion (Lovers) Balanced → Abstraction (Devil):**
"The love is infinite. But does it have a spine? Understand the flow. Feeling without Abstraction is chaos."

---

## PART VIII: VERIFICATION EXAMPLES

### Rebalancer Examples

**Example 1: Too Little 6 of Wands (Recognition)**
1. Associated Archetype: Sacrifice (Hanged Man)
2. Too Little → Vertical partner: Fortitude (Strength)
3. 6 of Wands is Outer → flip to Inner
4. Fortitude's Inner bound: **5 of Pentacles**
✓ Too Little 6 of Wands → 5 of Pentacles

**Example 2: Too Little Ace of Pentacles (Definition)**
1. Associated Archetype: Will (Magician)
2. Too Little → Vertical partner: Actualization (Sun)
3. Ace is Inner → flip to Outer
4. Actualization's Outer bound: **10 of Swords**
✓ Too Little Ace of Pentacles → 10 of Swords

### Growth Opportunity Examples

**Example 3: Balanced 3 of Wands (Assertion)**
1. Associated Archetype: Order (Emperor)
2. Balanced → Growth partner: Wisdom (High Priestess)
3. 3 of Wands is Inner → flip to Outer
4. Wisdom's Outer bound: **9 of Swords**
✓ Balanced 3 of Wands → 9 of Swords

**Example 4: Balanced Queen of Wands**
1. Associated Archetype: Drive (Chariot)
2. Balanced → Growth partner: Imagination (Moon)
3. Queen is Outer → flip to Inner
4. Imagination's Inner agent: **Page of Cups**
✓ Balanced Queen of Wands → Page of Cups

Wait — let me verify this. Queen is Outer polarity. Imagination's agents are:
- Page of Cups (Inner)
- No Outer agent (Imagination only has Page)

Hmm, that's a problem. Let me reconsider...

Actually, looking at the Archetype → Bounds/Agents table:
- Imagination (18) has: Page of Cups (Inner), and NO Outer Agent

So if we flip from Outer → Inner, we get Page of Cups. But what if the source is Inner (Knight of Cups)?
- Culture (5) → Tune (14)
- Knight is Inner → flip to Outer
- Tune's Outer agent: Queen of Swords ✓

**Example 5: Balanced 4 of Cups (Reverie)**
1. Associated Archetype: Compassion (Lovers) — DIAGONAL
2. Balanced → Polarity partner: Abstraction (Devil)
3. 4 of Cups is Inner → flip to Outer
4. Abstraction's Outer bound: **8 of Swords**
✓ Balanced 4 of Cups → 8 of Swords

---

## NOTES ON GESTALT ARCHETYPES

Potential (0), Will (1), Actualization (19), Recognition (20), Source (10), and Creation (21) do not participate in the Transpose/Polarity growth system because:
- Gestalt archetypes (0, 1, 19, 20) sit above the manifest matrix
- Portal archetypes (10, 21) are the frame, not the content

Their Bounds (Aces and 10s) therefore have no growth opportunity defined. When balanced, they represent completion points rather than launchpads.

---

*The structure is the authority. This table is derived from the Archetype correction pairs (diagonal/vertical/reduction) and growth pairs (transpose/polarity) with polarity flip for Bounds and Agents.*

*January 11, 2026*
