...
import { getFullCorrection } from './corrections.js';
...
export // 19/21 = diagonal-pair sums; 20 = vertical/axis-pair sum (10+10, Recognition); 40 = the Forty-Fold Seal's row constant (every row/column/2x2 toroidal block sums to 40)
const MEANINGFUL_SUMS = new Set([19, 20, 21, 40]);
...
export function rebalancerFor(signatureId, status) {
  ...
  // SOURCE OF TRUTH (2026-08-19, Air's catch): the LIVE corrections tables, not
  // the generated JSON snapshot — the snapshot predates the agent role-preservation
  // fixes and served role-flipped medicines into the computed-geometry lines while
  // the prose used the cured tables. Live lookup first; JSON only as fallback.
  let targetId = c.targetId ?? c.target?.id ?? null;
  let pathway = c.type ?? (status === 1 ? 'GROWTH' : status === 2 ? 'DIAGONAL' : status === 3 ? 'VERTICAL' : 'REDUCTION');
  try {
    const live = getFullCorrection(signatureId, status);
    if (live) {
      const liveTarget = live.targetAgentId ?? live.targetId ?? live.target;
      if (liveTarget != null && !live.isSelf) targetId = liveTarget;
      if (live.isSelf) targetId = signatureId;
      if (live.type) pathway = live.type;
    }
  } catch { /* JSON fallback stands */ }
  const target = targetId != null ? sigLite(targetId) : null;
  return {
    pathway,
    raw: { ...c, correctionType: pathway },
    target,
...
// Human-readable geometry block for a card's ARCHITECTURE section — DETERMINISTIC (no AI),
// so it is always accurate and free. This is the proof-of-work the dossier leaves in the reading.
export function formatCardGeometry(d) {
  if (!d) return '';
  const L = ['**Geometry (computed):**'];
  const t = d.transient;
  if (t) L.push(`- ${t.name}: ${t.class}${t.agentRank ? ` (${t.agentRank})` : ''} · ${t.house}${t.channel ? '/' + t.channel : ''} · ${t.stage ?? '—'} stage · ${t.innerOuter} · ${t.operation ?? '—'}${t.flags.portal ? ' · PORTAL' : ''}${t.flags.originalTriplet ? ' · original triplet (9·10·11)' : ''}`);
  const r = d.cardToPosition;
  if (r) {
    const bits = [];
    if (r.meaningfulSum) bits.push(`sum ${r.sum} — meaningful`);
    if (r.axisPair || r.anchorAxisPair) bits.push('axis pair (sum 20)');
    if (r.diagonalPair) bits.push(`diagonal pair (${r.sum}, same house)`);
    if (r.sameHouse && !r.diagonalPair) bits.push(`same house: ${r.sameHouse}`);
    if (r.sameStage) bits.push(`same stage: ${r.sameStage}`);
    if (r.displacement?.computable) bits.push(`displacement ${r.displacement.distance}-bit (${r.displacement.distanceClass})`);
    if (r.gridAdjacency) bits.push(`grid ${r.gridAdjacency.type}: ${r.gridAdjacency.medicineClass ?? r.gridAdjacency.moveClass}`);
    if (bits.length) L.push(`- Card ↔ position: ${bits.join(' · ')}`);
  }
  const rb = d.rebalancer;
  if (rb?.target) L.push(`- Medicine: ${rb.target.name} via ${(rb.raw?.correctionType ?? rb.pathway ?? '').toString().toUpperCase()}${rb.distance?.computable ? ` — ${rb.distance.distance}-bit (${rb.distance.distanceClass})` : ''}${rb.medicineFlags?.originalTriplet ? ' · the medicine is an original-triplet member' : ''}${rb.medicineFlags?.portal ? ' · the medicine is a PORTAL' : ''}`);
  for (const o of d.relationsToOtherCards ?? []) {
    const rr = o.relation; if (!rr) continue;
    const bits = [];
    if (rr.meaningfulSum) bits.push(`sum ${rr.sum} — meaningful`);
    if (rr.axisPair) bits.push('axis pair (sum 20)');
    if (rr.reductionKin) bits.push('reduction kin (shared digit root)');
    if (rr.sameHouse) bits.push(`same house: ${rr.sameHouse}`);
    if (rr.sameStage) bits.push(`same stage: ${rr.sameStage}`);
    if (rr.displacement?.computable) bits.push(`${rr.displacement.distance}-bit (${rr.displacement.distanceClass})`);
    if (bits.length) L.push(`- With ${o.otherTransient}: ${bits.join(' · ')}`);
  }
  for (const h of d.literalNameHits ?? []) L.push(`- Literal-name hit: "${h.matched}" appears in the question — high signal`);
