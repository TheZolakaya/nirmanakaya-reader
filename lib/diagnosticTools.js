...
export function buildTriageSeed(triage, drawMap) {
  ...
  if (l2_processes.byStage && Object.keys(l2_processes.byStage).length > 0) {
    ...
    for (const stage of ['Seed', 'Bridge', 'Fruition', 'Feedback']) {
  ...
  // ── PORTAL CHAINS ──
  // Trace Source (10) and Creation (21) as transient visitors and hosts
  if (drawMap) {
    lines.push('PORTAL CHAINS:');
    for (const [label, portalId] of [['Source (10)', 10], ['Creation (21)', 21]]) {
      const portalDraw = drawMap[portalId];
      if (portalDraw) {
        const visitorId = portalDraw.transient;
        const visitorName = getSignatureName(visitorId);
        const visitorHome = resolveArchetypePosition(visitorId);
        lines.push(`  ${label} hosts: ${visitorName} (id ${visitorId})`);
        if (visitorHome != null && visitorHome !== portalId) {
          const homeDraw = drawMap[visitorHome];
          if (homeDraw) {
            const homeVisitor = getSignatureName(homeDraw.transient);
            const homeStatus = STATUSES[homeDraw.status]?.name;
            lines.push(`    → visitor's home (${ARCHETYPES[visitorHome]?.name || visitorHome}): occupied by ${homeVisitor} — ${homeStatus}`);
          }
        }
      }
      // Where did this portal go as a transient?
      let landedAt = null;
      for (const [posStr, draw] of Object.entries(drawMap)) {
        if (draw.transient === portalId) {
          landedAt = Number(posStr);
          break;
        }
      }
      if (landedAt != null) {
        const landedStatus = STATUSES[drawMap[landedAt].status]?.name;
        const landedName = getSignatureName(landedAt);
        lines.push(`  ${label} visiting: ${landedName} (pos ${landedAt}) — ${landedStatus}`);
      }
    }
    lines.push('');

    // ── MAGNITUDE SIGNALS ──
    const balancedArchetypes = [];
    const selfSeated = [];
    const majorOnMajor = [];
    const soulHouseIds = [0, 1, 19, 20];
    const portalIds = [10, 21];

    for (let id = 0; id < 22; id++) {
      const draw = drawMap[id];
      if (!draw) continue;
      if (draw.status === 1) balancedArchetypes.push(id);
      if (draw.transient === id) {
        selfSeated.push({ id, name: ARCHETYPES[id]?.name, isMajor: portalIds.includes(id) || soulHouseIds.includes(id) });
      }
      if (draw.transient < 22) {
        const isPortal = portalIds.includes(id) || portalIds.includes(draw.transient);
        const isSoul = soulHouseIds.includes(id) || soulHouseIds.includes(draw.transient);
        if (id !== draw.transient) {
          majorOnMajor.push({
            durable: ARCHETYPES[id]?.name,
            visitor: ARCHETYPES[draw.transient]?.name,
            atPortal: isPortal,
            atSoul: isSoul
          });
        }
    ...
    lines.push('MAGNITUDE SIGNALS:');
    lines.push(`  Balanced archetypes: ${balancedArchetypes.length}/22`);
    if (selfSeated.length > 0) {
      const majors = selfSeated.filter(s => s.isMajor);
      const others = selfSeated.filter(s => !s.isMajor);
      if (majors.length > 0) lines.push(`  Self-seated (critical): ${majors.map(s => s.name).join(', ')}`);
      if (others.length > 0) lines.push(`  Self-seated (other): ${others.map(s => s.name).join(', ')}`);
    }
    const notablePairings = majorOnMajor.filter(p => p.atPortal || p.atSoul);
    if (notablePairings.length > 0) {
      lines.push('  Major-on-major pairings:');
      for (const p of notablePairings) {
        const tag = p.atPortal ? ' [PORTAL]' : p.atSoul ? ' [SOUL]' : '';
        lines.push(`    ${p.durable} ← ${p.visitor}${tag}`);
      }
    }
    if (majorOnMajor.length > notablePairings.length) {
      lines.push(`  Other major-on-major: ${majorOnMajor.length - notablePairings.length} additional pairings`);
    }
    lines.push('');

    // ── BEING HEALTH ──
    lines.push('BEING HEALTH (What? — 4 groups × 4 archetypes):');
    for (const [groupName, group] of Object.entries(BEING_GROUPS)) {
      let balanced = 0;
      let learning = 0;
      const memberLines = [];
      for (const memberId of group.members) {
        const draw = drawMap[memberId];
        if (!draw) continue;
        const status = STATUSES[draw.status]?.name;
        if (status === 'Balanced') balanced++;
        else learning++;
        memberLines.push(`${ARCHETYPES[memberId]?.name} [${status}]`);
      }
      lines.push(`  ${groupName} (${group.verb}): ${memberLines.join(', ')} [${balanced}B/${learning}L]`);
    }
    lines.push('');

    // ── IDENTITY HEALTH ──
    lines.push('IDENTITY HEALTH (Who? — 4 groups × 4 archetypes):');
    for (const [groupName, group] of Object.entries(IDENTITY_GROUPS)) {
      let balanced = 0;
      let learning = 0;
      const memberLines = [];
      for (const memberId of group.members) {
...
function getSignatureName(id) {
  if (id < 22) return ARCHETYPES[id]?.name || `Archetype ${id}`;
