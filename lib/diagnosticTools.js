...
export function buildTriageSeed(triage) {
  ...
  for (const [house, data] of Object.entries(l2_processes.byHouse)) {
    lines.push(`    ${house}: ${data.healthy} healthy, ${data.distorted} distorted`);
  }
  lines.push('');
  ...
  // Healthy houses (all members healthy)
  for (const [house, data] of Object.entries(l2_processes.byHouse)) {
    if (data.distorted === 0 && data.healthy > 0) {
      greenFlags.push(`HOUSE FULLY HEALTHY: ${house} — all ${data.healthy} processes balanced.`);
    }
  }
