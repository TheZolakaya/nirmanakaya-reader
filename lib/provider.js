...
const OR_ORDER = (process.env.OPENROUTER_PROVIDER_ORDER || 'Together').split(',').map((s) => s.trim()).filter(Boolean);
...
const openrouterExtras = (body, session) => ({
  provider: { order: OR_ORDER, allow_fallbacks: false, data_collection: 'deny', ...(OR_QUANT.length ? { quantizations: OR_QUANT } : {}) },
