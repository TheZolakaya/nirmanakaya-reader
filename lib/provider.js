...
// THE HOST (.499, raced 2026-09-20 on the same 900-token generation): Together 2.7s at 276 tok/s; Modal 3.4s;
// Venice 4.3s; Fireworks 9.5s; DeepInfra 15.8s at 47 tok/s (cheapest, but a person waited 20s for an opening).
// Together's cache-read rate is $0.006/M; a warm opening costs ~0.18c there vs 0.07c on DeepInfra — the right
// trade for someone waiting. Together does not declare its precision, so the quantization filter is OFF by
// default (set OPENROUTER_QUANTIZATIONS=fp8,bf16,fp16 to insist, which excludes Together/Fireworks/Modal).
// .540: THE HOSTS, ONE AT A TIME. Raced 2026-09-22 (22 hosts, two rounds, the real opening): Together, Parasail and Novita
// are the only ones under 3s with a working cache. provider.order would only move on an error; slowness stayed with the
// first host (a warm 579-token door took 20s). So the lane walks the list itself, one host per attempt, each on the short
// clock below; the next host is the retry; the ladder (DeepSeek direct, Anthropic) is behind them all.
const OR_ORDER = (process.env.OPENROUTER_PROVIDER_ORDER || 'Together,Parasail,Novita').split(',').map((s) => s.trim()).filter(Boolean);
const HOST_TIMEOUT_MS = Number(process.env.OPENROUTER_HOST_TIMEOUT_MS) || 15000; // 15s: a healthy host writes a whole opening in ~6s; a host at a third of its speed is abandoned and the next one takes it
const OR_QUANT = (process.env.OPENROUTER_QUANTIZATIONS || '').split(',').map((s) => s.trim()).filter(Boolean);
...
const openrouterExtras = (body, session, host = null) => ({
  provider: { order: host ? [host] : OR_ORDER, allow_fallbacks: false, data_collection: 'deny', ...(OR_QUANT.length ? { quantizations: OR_QUANT } : {}) },
  session_id: sessionFor(body, session),
});

/** The body as a given lane receives it. */
const forLane = (lane, body, session, host = null) => (lane === 'anthropic' ? body : {
  ...body,
  model: LANE_DEFS[lane].model(body.model),
  thinking: body.thinking || { type: 'disabled' },
  messages: withholdPersonalContext(body.messages, lane),
  ...(lane === 'openrouter' ? openrouterExtras(body, session, host) : {}),
});

async function sendRaw(lane, body, { beta, timeoutMs = TIMEOUT_MS } = {}) {
  ...
  if (lane !== 'anthropic') {
    ...
    const timer = setTimeout(() => ac.abort(new Error(`${lane} did not answer within ${timeoutMs / 1000}s`)), timeoutMs);
    try {
...
// .540: THE HOST WALK. One host per attempt, each pinned alone on the short clock; a stall or an error hands to the next.
// Returns { data, host, model } or null when every host failed (the reasons are logged; the caller trips the lane).
async function sendOpenRouter(body, session, { beta, tag = 'provider' } = {}) {
  const reasons = [];
  for (const host of OR_ORDER) {
    const pinned = forLane('openrouter', body, session, host);
    const t0 = Date.now();
    try {
      const { data } = await sendRaw('openrouter', pinned, { beta, timeoutMs: HOST_TIMEOUT_MS });
      if (looksAnswered(data)) {
        console.log(`[provider:${tag}] openrouter/${host} ${pinned.model} served in ${Date.now() - t0}ms (${data.usage?.output_tokens ?? '?'} out${data.usage?.cache_read_input_tokens ? `, ${data.usage.cache_read_input_tokens} cached` : ''}${typeof data.usage?.cost === 'number' ? `, $${data.usage.cost.toFixed(6)}` : ''}${data.id ? `, ${data.id}` : ''})`);
        return { data: textFirst(data), host, model: data.model || pinned.model };
      }
      reasons.push(`${host}: ${data?.error?.message || 'no content'}`);
      console.warn(`[provider:${tag}] openrouter/${host} gave no answer after ${Date.now() - t0}ms — next host`);
    } catch (e) {
      reasons.push(`${host}: ${e.message}`);
      console.warn(`[provider:${tag}] openrouter/${host} ${e.message} after ${Date.now() - t0}ms — next host`);
    }
  }
  console.warn(`[provider:${tag}] every OpenRouter host failed — ${reasons.join(' · ')}`);
  return null;
}

/** Walk the ladder. Returns { data, provider, model, ok }. */
export async function callProvider(body, { beta, tag = 'provider', session, only = null } = {}) {
  // `only`: a caller may restrict this one call to named lanes (.504: the sensitive turn goes to Anthropic)
  for (const lane of (Array.isArray(only) && only.length ? lanes().filter((l) => only.includes(l)) : lanes())) {
    ...
    if (lane === 'openrouter') { // .540: one host at a time
      const got = await sendOpenRouter(body, session, { beta, tag });
      if (got) return { data: got.data, provider: `openrouter/${got.host}`, model: got.model, ok: true };
      trip(lane, `[${tag}] every host failed`);
      continue;
    }
    try {
      const { data } = await sendRaw(lane, sent, { beta });
...
export async function providerFetch(url, options = {}, maxRetries = 3) {
  ...
  for (const lane of lanes()) {
    ...
    if (lane === 'openrouter') { // .540: one host at a time
      const got = await sendOpenRouter(body, body?.metadata?.user_id, { beta, tag: 'providerFetch' });
      if (got) return new Response(JSON.stringify(got.data), { status: 200, headers: { 'content-type': 'application/json' } });
      trip(lane, '[providerFetch] every host failed');
      continue;
    }
    try {
      const { data } = await sendRaw(lane, sent, { beta });
