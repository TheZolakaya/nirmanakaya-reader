...
const forLane = (lane, body) => (lane === 'anthropic' ? body : {
  ...body,
...
export async function callProvider(body, { beta, tag = 'provider' } = {}) {
  for (const lane of lanes()) {
    ...
    try {
      const { data } = await sendRaw(lane, sent, { beta });
      if (looksAnswered(data)) { console.log(`[provider:${tag}] ${lane} ${sent.model} served (${data.usage?.output_tokens ?? '?'} out)`); return { data: textFirst(data), provider: lane, model: data.model || sent.model, ok: true }; }
...
export async function providerFetch(url, options = {}, maxRetries = 3) {
  ...
  for (const lane of lanes()) {
    ...
    try {
      const { data } = await sendRaw(lane, sent, { beta });
      if (looksAnswered(data)) { console.log(`[provider] ${lane} ${sent.model} served (${data.usage?.output_tokens ?? '?'} out)`); return new Response(JSON.stringify(textFirst(data)), { status: 200, headers: { 'content-type': 'application/json' } }); }
...
export async function createMessage(params, { tag = 'sdk' } = {}) {
  for (const lane of lanes()) {
    ...
    try {
      ...
      const r = await clients[lane].messages.create(forLane(lane, params));
