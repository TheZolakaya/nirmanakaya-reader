...
export function seedParts({ question = null, draws = [], index = 0 } = {}) {
  const draw = draws[index];
  if (!draw) return { block: '', lines: '' };
  const L = [];
  // 1. the geometry — computed, rendered as lines
  try {
    const cards = drawsToCards(draws);
    const d = buildCardDossier({ question, cards, index });
    const geo = formatCardGeometry(d).split('\n').filter((l) => l.startsWith('- ')).map((l) => canonName(l.slice(2)));
  ...
  try {
    ...
    if (draw.transient <= 21) {
      ...
      if (route?.name && medicine && canonName(route.name) === medicine && route.instruction) T.push(`Retrieval: ${canonName(route.instruction)}`);
