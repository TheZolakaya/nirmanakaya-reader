...
export function filterProhibitedTerms(text) {
  ...
  // Scrub leaked generation scaffolding wherever it survives (Haiku emits
  // [signature:N:...] marker tokens inside prose more than Sonnet did)
  cleaned = cleaned.replace(/\[signature:[^\]]{0,60}\]/gi, '');

  // VOCATIVE-ONLY stripping (2026-08-16 fix). The old anywhere-match ate legitimate
  // mid-sentence words — "That's beautiful." lost its object, "dear to me" broke,
  // "handsome margin" would too. Endearments are only a violation when used as
  // ADDRESS: "Oh honey, ..." / sentence-start "Honey, ..." / ", honey." — so those
  // are the only shapes removed. Adjective/noun uses survive.
  PROHIBITED_TERMS.forEach(term => {
    cleaned = cleaned.replace(new RegExp(`\\bOh\\s+${term}\\b[,.!]?\\s*`, 'gi'), '');
    cleaned = cleaned.replace(new RegExp(`(^|[.!?]\\s+)${term}\\s*[,—–-]\\s*`, 'gi'), '$1');
    cleaned = cleaned.replace(new RegExp(`,\\s*${term}\\s*([.!?,])`, 'gi'), '$1');
  });

  // Verb shift: "card" → "signature" (system uses signatures, not cards)
  cleaned = cleaned.replace(/\bthe cards\b/gi, 'the signatures');
  cleaned = cleaned.replace(/\bthese cards\b/gi, 'these signatures');
  cleaned = cleaned.replace(/\bthose cards\b/gi, 'those signatures');
  cleaned = cleaned.replace(/\bboth cards\b/gi, 'both signatures');
  cleaned = cleaned.replace(/\ball cards\b/gi, 'all signatures');
  cleaned = cleaned.replace(/\byour cards\b/gi, 'your signatures');
