...
// Assembly order: BASE_SYSTEM (first — stable prefix for prompt caching) →
// Posture → Locus → Architecture → FORMAT → WHY → Voice (last)
// BASE_SYSTEM MUST stay first: lib/cachedSystem.js splits on it server-side
// so all users share one prompt-cache entry regardless of dial settings.
...
export function buildSystemPrompt(userLevel, options = {}) {
  ...
  // Assembly: Kernel (cache prefix) → Posture → Locus → Architecture → Format → Why → Voice (last)
