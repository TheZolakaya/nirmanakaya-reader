...
// Assembly order: BASE_SYSTEM → FORMAT (both cache prefix) →
// Posture → Locus → Architecture → WHY → Voice (last)
// BASE_SYSTEM MUST stay first and FORMAT immediately after it:
// lib/cachedSystem.js splits on both server-side so all users share cache
// entries regardless of dial settings (format has only 3 length variants).
...
export function buildSystemPrompt(userLevel, options = {}) {
  ...
  // Locus injection — right after the posture header so it frames the interpretation
  // (moved after BASE_SYSTEM 2026-07-01 to keep the cacheable prefix stable)
  ...
  const archBlock = showArchitecture
    ? '\nARCHITECTURE VISIBLE: Use structural terminology freely — house names, channel names, process stages (Seed/Bridge/Fruition/Feedback), Inner/Outer horizon, Archetype/Bound/Agent types, rebalancer geometry (diagonal/vertical/reduction), channel resonance/crossing. The user wants to see the scaffolding.\n'
  ...
  // Assembly: Kernel + Format (cache prefix) → Posture → Locus → Architecture → Why → Voice (last)
  return `${BASE_SYSTEM}\n\n${formatBlock}\n\n${header}\n\n${locusBlock}${archBlock}\n\n${WHY_MOMENT_PROMPT}\n\n${personaPrompt}`;
}
...
export function getAPIConfig(userLevel, useHaiku = true) {
  ...
  return {
    model: useHaiku ? MODEL_IDS.haiku : MODEL_IDS.sonnet,
    max_tokens: 16000
  };
}
