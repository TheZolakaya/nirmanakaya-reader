...
IMPORTANT: Interpret "${trans.name}" — this is the signature they drew. Start your response with "${trans.name}".`;
  ...
  // Build the section marker instructions based on the number of cards
  const cardMarkerList = draws.map((_, i) => {
    const n = i + 1;
    return `[CARD:${n}:SURFACE], [CARD:${n}:WADE], [CARD:${n}:SWIM], [CARD:${n}:ARCHITECTURE], [CARD:${n}:MIRROR], [CARD:${n}:WHY:SURFACE], [CARD:${n}:WHY:WADE], [CARD:${n}:WHY:SWIM], [CARD:${n}:WHY:DEEP], [CARD:${n}:WHY:ARCHITECTURE]` +
           (draws[i].status !== 1 ? `, [CARD:${n}:REBALANCER:SURFACE], [CARD:${n}:REBALANCER:WADE], [CARD:${n}:REBALANCER:SWIM], [CARD:${n}:REBALANCER:ARCHITECTURE]` : '');
  }).join(', ');
...
[LETTER:SURFACE], [LETTER:WADE], [LETTER:SWIM], [SUMMARY], ${cardMarkerList}, [PATH:SURFACE], [PATH:WADE], [PATH:SWIM], [PATH:ARCHITECTURE] (if 2+ imbalanced).

Each marker on its own line. Generate ALL depth levels for Letter, Path, each card (including WHY with Surface/Wade/Swim/Deep), and Rebalancer (if imbalanced).`;
}
