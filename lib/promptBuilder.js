...
import { buildPersonaPrompt, getLengthOverride } from './personas.js';
...
export function buildSystemPrompt(userLevel, options = {}) {
  ...
  // Length override — goes after FORMAT_INSTRUCTIONS to override word targets
  const lengthBlock = getLengthOverride(readingLength);

  // Assembly: Posture → Locus → Kernel → Architecture → Format → Length → Why → Voice (last)
  return `${header}\n\n${locusBlock}${BASE_SYSTEM}\n\n${archBlock}\n\n${FORMAT_INSTRUCTIONS}${lengthBlock ? '\n\n' + lengthBlock : ''}\n\n${WHY_MOMENT_PROMPT}\n\n${personaPrompt}`;
}
