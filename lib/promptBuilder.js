...
IMPORTANT: Interpret "${trans.name}" — this is the signature they drew. Start your response with "${trans.name}".`;
  ...
  const cardMarkerList = draws.map((_, i) => {
    const n = i + 1;
    return `[CARD:${n}:SURFACE], [CARD:${n}:WADE], [CARD:${n}:SWIM], [CARD:${n}:ARCHITECTURE], [CARD:${n}:MIRROR]` +
...
[LETTER], [SUMMARY], ${cardMarkerList}, [PATH:SURFACE], [PATH:WADE] (if 2+ imbalanced), [WORDS_TO_WHYS].

Each marker on its own line. Generate ALL depth levels (Surface, Wade, Swim) for each card and its Rebalancer (if imbalanced).`;
}
