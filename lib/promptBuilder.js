...
[LETTER:SURFACE], [LETTER:WADE], [LETTER:SWIM], [LETTER:DEEP], [SUMMARY], ${cardMarkerList}, [PATH:SURFACE], [PATH:WADE], [PATH:SWIM], [PATH:DEEP], [PATH:ARCHITECTURE] (if 2+ imbalanced), [FULL_ARCHITECTURE].

Each marker on its own line. Generate ALL depth levels for Letter, Path, each card (including WHY with Surface/Wade/Swim/Deep), and Rebalancer (if imbalanced). The DEEP level has no sentence limits and can use framework terminology if interpreted inline. FULL_ARCHITECTURE shows complete derivation chains for all cards.`;
}
