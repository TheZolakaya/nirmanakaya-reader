...
// includeAll=true: all users with email from auth.users (F&F mode)
// includeAll=false: only opted-in users from profiles table
export async function getUsersForBroadcast(supabaseAdmin, includeAll = false) {
  ...
  if (includeAll) {
    // F&F mode: get ALL users from auth.users (includes Google OAuth users)
    try {
      const { data: { users }, error } = await supabaseAdmin.auth.admin.listUsers();
      if (error) return { data: [], error: error.message };

      // Filter to users with email and map to expected format
  ...
  // Standard mode: only opted-in users from profiles
