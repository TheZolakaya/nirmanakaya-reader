...
// Where to land after an OAuth round trip. Without this, signing in from anywhere other than
// the home page dumped people back on "/" and they had to retype the address they came from.
export function rememberAuthReturn(path) {
  try {
    ...
    if (p && !p.startsWith('/auth')) sessionStorage.setItem('nkya_auth_return', p);
  } catch {}
}

export function takeAuthReturn() {
  try {
    const p = sessionStorage.getItem('nkya_auth_return');
    sessionStorage.removeItem('nkya_auth_return');
...
// Save a reading (unified: writes to user_readings)
export async function saveReading(reading) {
  if (!supabase) return { error: 'Supabase not configured' };
  // .547: A LOCAL TEST NEVER SAVES INTO A REAL ACCOUNT. Test readings run on localhost (signed in as the founder) were
  // saved as his, summarised into his journey thread, and the suggester offered him a question about "Dan" — a name
  // from a test. On localhost nothing is saved unless the page is opened with ?save=1.
  if (typeof window !== 'undefined' && /^(localhost|127\.0\.0\.1)$/.test(window.location.hostname) && !/[?&]save=1/.test(window.location.search)) {
    console.warn('[saveReading] localhost — not saved (open with ?save=1 to save)');
  ...
  const modelKey = reading.model || 'sonnet';
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      card_count: reading.cards?.length || 1,
      voice: reading.voice || 'friend',
      draws: reading.cards, // Raw draw objects
      interpretation: {
        letter: reading.letter,
        synthesis: reading.synthesis,
        cards: [] // Filled in later by updateReadingContent
      },
...
// Update reading content (cards with interpretations, synthesis, threads, expansions,
// verdict + posture — the Integrate answer and the mode that produced the reading)
export async function updateReadingContent(readingId, { cards, synthesis, letter, thread_data, verdict, posture, yieldData, usage, plainAnswer }) {
  if (!supabase || !readingId) return { error: 'Missing required params' };

  const { user } = await getUser();
  if (!user) return { error: 'Not authenticated' };

  // Read current interpretation to merge
  ...
  const existingInterp = current?.interpretation || {};
  const updatedInterp = { ...existingInterp };

  if (cards !== undefined) {
    // cards = array of draw objects with .interpretation attached
    updatedInterp.cards = cards.map(c => ({
      interpretation: c.interpretation,
      rebalancing: c.interpretation?.rebalancer || c.interpretation?.rebalancing || null,
      frameLabel: c.frameLabel ?? null,
      frameLens: c.frameLens ?? null
    }));
  }
  if (synthesis !== undefined) updatedInterp.synthesis = synthesis;
  if (letter !== undefined) updatedInterp.letter = letter;
  if (thread_data !== undefined) updatedInterp.thread_data = thread_data;
  if (verdict !== undefined) updatedInterp.verdict = verdict;
  if (posture !== undefined) updatedInterp.posture = posture;
  if (yieldData !== undefined) updatedInterp.yield = yieldData;
  if (usage !== undefined) updatedInterp.usage = usage; // {tokens, model, estCostUSD} — the cost telemetry
  if (plainAnswer !== undefined) updatedInterp.plainAnswer = plainAnswer; // the reading's spoken last word

  const updateData = { interpretation: updatedInterp };
  // Also update draws with full card data if provided
  if (cards !== undefined) updateData.draws = cards;
...
// Get a public reading by share token
...
export async function setReadingPublic(id, isPublic, content = null) {
  ...
  // If content provided, merge into interpretation
  if (content) {
    const { data: current } = await supabase
      .from('user_readings')
      .select('interpretation')
    ...
    const interp = { ...(current?.interpretation || {}) };
    if (content.synthesis) interp.synthesis = content.synthesis;
    if (content.letter) interp.letter = content.letter;
    updateData.interpretation = interp;
    if (content.cards) updateData.draws = content.cards;
  }
...
export async function getUserPublicReadings(userId, limit = 20) {
  ...
  const { data, error } = await supabase
    .from('user_readings')
    .select('id, topic, mode, spread_type, created_at, share_token')
    .eq('user_id', userId)
    .eq('is_public', true)
    .order('created_at', { ascending: false })
    .limit(limit);
...
// Update a discussion (owner or admin — RLS enforced)
export async function updateDiscussion(id, { title, content }) {
  if (!supabase) return { error: 'Supabase not configured' };
...
// Update a reply (owner or admin — RLS enforced)
...
export async function getAdminStats() {
  ...
  // Get token usage per user from user_readings (unified table)
...
// Check if there's recent community activity (messages in last N minutes)
export async function checkCommunityActivity(minutesAgo = 15) {
  ...
  if (error) return { hasActivity: false, error };
  return { hasActivity: count > 0, recentCount: count };
}

// Subscribe to global community presence (anyone online in any room)
// Returns channel - call channel.unsubscribe() to clean up
export function subscribeToGlobalPresence(onSync) {
...
export async function trackGlobalPresence(channel, user, displayName) {
  await channel.track({
    user_id: user.id,
    display_name: displayName || user.email?.split('@')[0] || 'Anonymous',
    online_at: new Date().toISOString()
...
// Subscribe to any new chat message (for header indicator)
export function subscribeToGlobalMessages(onMessage) {
  const channel = supabase
...
export async function createChatRoom(name, description = '', isPrivate = false) {
  const { user } = await getUser();
  if (!user) return { data: null, error: 'Not authenticated' };
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      is_default: false,
      created_by: user.id,
      sort_order: 999 // User rooms sort after default rooms
    })
    .select()
    .single();
  ...
  // If room created successfully, add creator as owner
  ...
  if (memberError) {
    // Rollback room creation if member insert fails
...
// Update a chat room
export async function updateChatRoom(roomId, updates) {
  const { data, error } = await supabase
    .from('chat_rooms')
    .update({
      name: updates.name,
...
// Invite user to private room (by user ID or email)
...
// Search users for invite (by display name)
...
export async function removeFromRoom(roomId, userId = null) {
  const { user } = await getUser();
  if (!user) return { error: 'Not authenticated' };

  // If no userId provided, remove self (leave room)
...
export async function isRoomMember(roomId) {
  const { user } = await getUser();
  ...
  return { isMember: !error && !!data, role: data?.role || null };
}

// Get rooms where user is a member (for private rooms)
...
// Get user's personal context facts
export async function getProfileContext() {
  ...
  if (!token) return { data: [], error: 'No session' };

  const res = await fetch('/api/user/profile-context', {
    headers: { 'Authorization': `Bearer ${token}` }
  });
  const result = await res.json();
  return { data: result.facts || [], error: result.error || null };
}

// Add a manual personal context fact
export async function addProfileContext(fact, category) {
  if (!supabase) return { data: null, error: 'Supabase not configured' };
  const session = await getSession();
  const token = session?.session?.access_token;
  ...
  const result = await res.json();
  return { data: result.fact || null, error: result.error || null };
}

// Archive (soft-delete) a personal context fact
export async function removeProfileContext(id) {
  if (!supabase) return { error: null };
  const session = await getSession();
  const token = session?.session?.access_token;
...
export async function setPersonalizationEnabled(enabled) {
  return updateProfile({ personalization_enabled: !!enabled });
}
