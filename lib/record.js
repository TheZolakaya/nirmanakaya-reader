...
export function drawRecord(d, DEFS) {
  ...
  const backLine = balancedFace ? `${balancedFace}${def?.class !== 'Archetype' && def?.associatedArchetypeName ? ` (its parent ${def.associatedArchetypeName}'s balanced face, which this card carries)` : ''}` : null;
  ...
  return [
    ...
    backLine ? `  WHEN IT IS BACK IN BALANCE (this card's own face — what the medicine is FOR; the other side): ${backLine}` : null,
