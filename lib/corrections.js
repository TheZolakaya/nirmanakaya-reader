...
export function getAgentCorrection(agent, status) {
  ...
  // - Diagonal (Too Much): SAME role (King stays King, Page stays Page)
  // - Vertical (Too Little): FLIP role (King → Page)
  // - Reduction (Unacknowledged): FLIP role (King → Page)
  const shouldFlipRole = archetypeCorrection.type !== 'diagonal';
