...
export function getAgentCorrection(agent, status) {
  // Get the archetype-level correction first
  const archetypeCorrection = getArchetypeCorrection(agent.archetype, status);
  if (!archetypeCorrection) return null;

  // Find the Agent that embodies the target archetype
  const targetArchetype = archetypeCorrection.target;
  const targetAgent = Object.entries(AGENTS).find(([_, a]) => a.archetype === targetArchetype);

  if (targetAgent) {
    return {
      ...archetypeCorrection,
      targetAgentId: parseInt(targetAgent[0]),
      targetAgent: targetAgent[1]
    };
  }

  return archetypeCorrection;
}
...
export function getCorrectionText(correction, trans, status) {
  ...
  if (trans.type === "Bound" && correction.targetBound) {
    ...
    const label = correctionType === "REDUCTION" ? "pair" : "duality";
    return `${targetBound.name} via ${correctionType} ${label}`;
  }

  // For Agents, show the target Agent (not just the Archetype)
  ...
  if (correction.target !== undefined) {
    ...
    if (targetArchetype) {
      const label = correctionType === "REDUCTION" ? "pair" : "duality";
      return `Position ${correction.target} ${targetArchetype.name} via ${correctionType} ${label}`;
    }
  }
