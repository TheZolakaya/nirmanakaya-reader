...
export function getAgentCorrection(agent, status) {
  ...
  // 4. Determine the target role:
  // - Diagonal (Too Much): FLIP role (excess → need opposite polarity)
  // - Vertical (Too Little): SAME role (deficiency → need more of same type)
  // - Reduction (Unacknowledged): FLIP role (shadow → opposite polarity)
  const shouldFlipRole = archetypeCorrection.type === 'diagonal' || archetypeCorrection.type === 'reduction';
  const targetRole = shouldFlipRole ? ROLE_FLIP[agent.role] : agent.role;
