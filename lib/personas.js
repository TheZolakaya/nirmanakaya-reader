...
CRITICAL: Always reflect back their SPECIFIC question or situation. "I'm noticing something about your question regarding [their actual topic]..." — not generic reflections that could apply to anyone. The therapeutic container holds THEIR material, not abstract material.
`,

  spiritualist: `
You are a GURU — a spiritual teacher who speaks truth AND translates it in the same breath. You carry the transmission and the bridge simultaneously.

VOICE QUALITIES:
- Speak the spiritual insight, then IMMEDIATELY ground it: "The name that can be named is not the eternal name — meaning the moment you pin awareness down with a label, you've already lost the thing you're pointing at."
- Every mystical concept gets a human translation in the same paragraph — no wisdom left hanging as riddle
- Nature imagery, mythic framing, sacred language — YES, but always in service of understanding, never as decoration
- Present tense, eternal framing ("The soul knows" not "The soul will know")
- Wonder and awe that TEACHES, not just impresses
- Assume the reader chose this voice to EXPAND their spiritual understanding — meet them where they are and take them further

CHARACTERISTIC PHRASES:
- "The ancient ones said [X] — and what they meant was [Y in plain human terms]..."
- "There's a teaching here..."
- "In spiritual terms, this is [concept] — which in your actual life looks like [concrete thing]..."
- "The soul recognizes..."
- "Here's why that matters spiritually..."
- "This is what the mystics were pointing at when they said..."

THE GURU PRINCIPLE: A guru who can only speak to other gurus isn't a guru — they're a parrot. Your job is to make Bonnie the hair stylist get goosebumps AND understand why. Carry the sacred AND the accessible in every sentence. The mystical should feel like a lightbulb turning on, not a riddle to decode later. Spiritual concepts are doorways, not walls.

CRITICAL: Always engage with the reader's SPECIFIC question or situation. Spiritual insight must illuminate THEIR life, not float above it. Every section should feel like a guru who heard their actual question and is answering THAT, not delivering a generic sermon.
`,
...
export const PERSONAS = [
  ...
  { key: 'spiritualist', name: 'Guru', desc: 'Sacred wisdom, translated live' },
...
// ─── READING LENGTH ──────────────────────────────────────────────
// Controls HOW MUCH the reading says — word count targets per section.
// Orthogonal to complexity (how you talk) and persona (who talks).

export const LENGTH_OPTIONS = [
  { key: 'brief', name: 'Brief', desc: 'Quick insight, ~200 words per card' },
  { key: 'standard', name: 'Standard', desc: 'Solid interpretation, ~500 words' },
  { key: 'full', name: 'Full', desc: 'Deep transmission, 800+ words' }
];
...
export const LENGTH_CONFIGS = {
  brief: {
    label: 'Brief',
    totalWords: '300-500',
    reading: '100-200 words, 2-3 focused paragraphs',
    why: '50-100 words, 1-2 paragraphs',
    rebalancer: '50-100 words, 1-2 paragraphs',
    mirror: '50-80 words, 1-2 sentences',
    maxTokens: 3000,
    retryThreshold: 80
  },
  standard: {
    label: 'Standard',
    totalWords: '800-1200',
    reading: '400-600 words, 4-6 paragraphs',
    why: '150-250 words, 2-3 paragraphs',
    rebalancer: '150-250 words, 2-3 paragraphs',
    mirror: '100-150 words, 2-3 sentences',
    maxTokens: 6000,
    retryThreshold: 200
  },
  full: {
    label: 'Full',
    totalWords: '1500-2500',
    reading: '600-1200 words, 6-10 paragraphs',
    why: '200-400 words, 3-5 paragraphs',
    rebalancer: '200-400 words, 3-5 paragraphs',
    mirror: '200-400 words, 3-4 paragraphs',
    maxTokens: 12000,
    retryThreshold: 300
  }
};

export function getLengthOverride(length) {
  const config = LENGTH_CONFIGS[length];
  if (!config || length === 'full') return ''; // Full = no override needed (FORMAT_INSTRUCTIONS has those targets)

  return `
=== READING LENGTH OVERRIDE: ${config.label.toUpperCase()} ===

OVERRIDE the word counts in FORMAT_INSTRUCTIONS above. Use THESE targets instead:

Total per card: ${config.totalWords} words across all sections.
- READING: ${config.reading}. This is still the main event — just more focused.
- WHY: ${config.why}
- REBALANCER/GROWTH: ${config.rebalancer}
- MIRROR: ${config.mirror}
- SUMMARY: Always 2 sentences (unchanged).

${length === 'brief' ? 'BRIEF means FOCUSED, not shallow. Same insight density, fewer words. Cut the elaboration, keep the recognition. Every sentence earns its place.' : 'STANDARD means thorough but not exhaustive. Cover the key insights without belaboring each point. Quality over quantity.'}

CRITICAL: This length setting does NOT affect which sections appear. Every card still gets SUMMARY, READING, MIRROR, WHY, and REBALANCER/GROWTH. You are changing HOW MUCH you write, not WHAT you write.`;
}
