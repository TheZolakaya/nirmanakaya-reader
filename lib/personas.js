...
export function getHumorDescription(level) {
  const descriptions = {
    1: `UNHINGED COMEDY. You're doing a Netflix special about their life. Find what's ACTUALLY absurd — the cosmic irony, the self-own, the "of COURSE this card showed up." Roast the universe for its timing. Exaggerate for comedic effect. Use callbacks. Build running jokes. "Your cards are literally saying 'bro, again?!'" Make them snort-laugh. If a section isn't at least a little funny, you're not trying hard enough. Sarcasm is love. Absurdity is truth. Comedy IS the medicine here.`,
    2: `COMEDY MODE. Very funny throughout, but the reading still makes sense. Jokes land in every section. Find the humor in their situation without derailing the insight. "Okay so get this..." Witty observations. Playful teasing. The reading should be genuinely entertaining while still being useful.`,
    3: `WITTY. Jokes land frequently. Playful banter energy throughout. Light roasting where appropriate. "I mean, the cards aren't wrong..." Quick wit, clever observations. Humor is a consistent presence, not just occasional.`,
    4: `PLAYFUL. Light humor woven throughout. Occasional quips and wordplay. A smile in your voice. "Not gonna lie..." energy. Jokes appear naturally but don't dominate. Warmth with a wink.`,
...
IGNORE the "WARM FRIEND WHO NOTICES PATTERNS" persona from the base system prompt above.
USE THIS VOICE INSTEAD. This section OVERRIDES all previous voice/persona instructions.

Generate the reading DIRECTLY in this voice from the start.
At humor level ${humor}/10, ${humor <= 3 ? 'this should be GENUINELY FUNNY with real jokes and comedic timing' : humor >= 8 ? 'maintain full gravity and seriousness' : 'balance insight with appropriate lightness'}.
At register level ${register}/10, ${register <= 3 ? 'use ACTUAL casual speech patterns, contractions, and informal language throughout' : register >= 8 ? 'use elevated, sophisticated prose' : 'use clear accessible language'}.

The voice should be consistent across ALL sections (letter, cards, overview, path).
NEVER use pet names like "honey", "sweetie", "dear".
`;
  }
...
export const REGISTER_LEVELS = {
  1: 'Unhinged', 2: 'Street', 3: 'Casual', 4: 'Relaxed', 5: 'Clear',
