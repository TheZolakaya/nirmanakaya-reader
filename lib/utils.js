...
export function formatDrawForAI(draws, spreadType, spreadKey, showTraditional) {
  ...
  return draws.map((draw, i) => {
    ...
    let domainContext, positionLens, positionName;
    if (isReflect && spreadConfig?.positions?.[i]) {
      positionName = spreadConfig.positions[i].name;
      domainContext = positionName;
      positionLens = spreadConfig.positions[i].lens;
    } else {
      positionName = draw.position !== null ? ARCHETYPES[draw.position]?.name : null;
      domainContext = positionName || 'this reading';
      positionLens = null;
    }
    ...
    // DEBUG: Log what we're sending for position context
    console.log(`[formatDrawForAI] Card ${i + 1}: draw.position=${draw.position}, positionName=${positionName}, trans.name=${trans.name}`);
