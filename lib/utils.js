    ...
    return {
      ...
      cards: legacy.cards.map((card, i) => {
        ...
        return {
          ...
          swim: card.content,
          architecture: null,
          mirror: card.whyMoment ? `${card.whyMoment.recognition || ''}\n${card.whyMoment.question || ''}`.trim() : null,
      ...
      wordsToWhys: legacy.wordsToWhys,
...
function parseProgressiveDepthResponse(responseText, draws) {
  const sections = {
    ...
    summary: null,
    cards: [],
    path: { surface: null, wade: null },
    wordsToWhys: null
  };

  // All possible markers for boundary detection
  const allMarkers = [
    'LETTER', 'SUMMARY', 'WORDS_TO_WHYS',
    'PATH:SURFACE', 'PATH:WADE',
    ...draws.flatMap((_, i) => {
      const n = i + 1;
      return [
  ...
  // Extract letter (first thing in new format)
  sections.letter = extractSection(responseText, 'LETTER', allMarkers.filter(m => m !== 'LETTER'));
  ...
  sections.path.surface = extractSection(responseText, 'PATH:SURFACE', [...pathEndMarkers, 'PATH:WADE']);
  sections.path.wade = extractSection(responseText, 'PATH:WADE', pathEndMarkers);

  // Extract words to whys
  sections.wordsToWhys = extractSection(responseText, 'WORDS_TO_WHYS', allMarkers.filter(m => m !== 'WORDS_TO_WHYS'));
