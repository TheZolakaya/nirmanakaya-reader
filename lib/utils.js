...
// THE RECORD RIDES ON THE DRAW TEXT (founder, 2026-09-17): one standard for what the Reader is told
// about a draw, for every caller — the summary, the letter, the path, the Why, the external port,
// the easy reader. Before this, five callers had four different answers.
...
export function generateSpread(count) {
  // Support 22 (archetype-only) or 78 (full state) readings
  const effectiveCount = Math.min(count, 78);
  const positionPool = shuffleArray([...Array(effectiveCount <= 22 ? 22 : 78).keys()]);
...
export function formatDrawForAI(draws, spreadType, spreadKey, showTraditional, depth = 'wade', frameLabels = null, frameLenses = null, includeRecord = true) {
...
Status: ${stat.name} — ${stat.desc}`;
    ...
    if (correctionText) {
      ...
      result += `\nRebalancer target: ${rebalancerTargetName} (the only card the rebalancer sections discuss; never quote this line).`;
    ...
    if (includeRecord) { try { result += `\n\n${drawRecord(draw, DEFS)}`; } catch {} }
    return result;
  }).join('\n\n');
}
...
// Residual-scaffolding scrub (2026-08-18): models (Haiku especially) emit marker-LIKE
// tokens the parser doesn't know — "[signature:0:SUMMARY]", "# [LETTER]",
// "## [SIGNATURE 1: ...]" — which then ride inside extracted content and render as
// leaked scaffolding. Known markers are consumed by extractSection; anything still
// marker-shaped after extraction is debris. Two moves: cut everything from the first
// leaked SIGNATURE/section block onward (they flood in trailing groups), then strip
// stray inline tokens.
function scrubResidualMarkers(content) {
  if (!content) return content;
  let c = content;
  const flood = c.search(/(^|\n)\s*#{0,6}\s*\[(signature|SIGNATURE|LETTER|CARD|SUMMARY|PATH)[\s:0-9]/);
  if (flood !== -1) c = c.slice(0, flood);
  c = c.replace(/#{0,6}\s*\[(signature|LETTER|CARD|SUMMARY|PATH|FULL_ARCHITECTURE)[^\]]{0,60}\]/gi, '');
  c = c.replace(/^\s*(?:LETTER|SUMMARY|PATH)\b[\s:—-]*/, ''); // .534: flash writes the marker without its brackets
  return c.trim();
}
...
function extractSection(text, startMarker, endMarkers = []) {
  ...
  return scrubResidualMarkers(text.slice(startIndex, endIndex).trim());
}
...
export // NOTE (2026-09-02 housecleaning): the surface/wade/swim/deep depth tiers are VESTIGIAL — V3 removed the depth param ("always deep"); all four tiers hold identical strings. Retained for saved-reading compatibility; slated for removal/repurposing in the M1 (One-Minute Mirror) build.
...
// .529: DIRECTIVE ECHO. Flash quoted 'REBALANCER TARGET: … You MUST discuss …' back into a rendered section (Keel,
// Sept 22 general reading). Any sentence carrying instruction vocabulary is dropped before a section reaches the
// page. Paragraph breaks are kept. Cheap insurance under the source fix (the lines are now facts in a wrapper).
const DIRECTIVE_RX = /(?:REBALANCER TARGET|REBALANCER CONTEXT|GROWTH TARGET|GROWTH OPPORTUNITY:|SECTION TARGETS|SECTION MINIMUMS|You MUST|never reproduce|\[\[instruction|read it there\b|This rebalancing (?:happens|is happening) in the)/;
export function stripDirectiveEcho(text) {
  if (!text || typeof text !== 'string') return text;
  return text.split(/\n\n+/).map((para) =>
    para.split(/(?<=[.!?])\s+(?=[A-Z"'\u201c(\[])/).filter((sent) => !DIRECTIVE_RX.test(sent)).join(' ').trim()
  ).filter(Boolean).join('\n\n');
}
