...
⚠️ INTERPRETATION KEY: This is ${trans.name} IN ${domainContext}. Your interpretation must explain what ${trans.name} means BECAUSE it landed in ${domainContext}. The position "${domainContext}" shapes the entire meaning. You MUST mention "${domainContext}" and explain how it contextualizes ${trans.name}.
Agency (THE SUBJECT): ${agencyInfo}
Domain (THE CONTEXT — where agency expresses): ${domainContext}
Status: ${stat.name} — ${stat.desc}${lensLine}
...
Grammar rule: ${trans.name} is the subject. Never say "${domainContext} is [doing X]" — say "${trans.name} is [doing X] in ${domainContext}".`;
  }).join('\n\n');
}
