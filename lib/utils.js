...
export function parseReadingResponse(responseText, draws) {
  ...
  if (!isProgressiveDepthFormat(responseText)) {
    ...
    return {
      letter: legacy.letter ? { surface: legacy.letter, wade: legacy.letter, swim: legacy.letter } : null,
...
function parseProgressiveDepthResponse(responseText, draws) {
  const sections = {
    letter: { surface: null, wade: null, swim: null },
    summary: null,
    cards: [],
    path: { surface: null, wade: null, swim: null, architecture: null }
  };

  // All possible markers for boundary detection
  const allMarkers = [
    'LETTER:SURFACE', 'LETTER:WADE', 'LETTER:SWIM',
    'SUMMARY',
    'PATH:SURFACE', 'PATH:WADE', 'PATH:SWIM', 'PATH:ARCHITECTURE',
    ...draws.flatMap((_, i) => {
      const n = i + 1;
      return [
        `CARD:${n}:SURFACE`, `CARD:${n}:WADE`, `CARD:${n}:SWIM`,
        ...
        `CARD:${n}:REBALANCER:SWIM`, `CARD:${n}:REBALANCER:ARCHITECTURE`
      ];
    })
  ];

  // Extract letter at all depths
  const letterEndMarkers = allMarkers.filter(m => !m.startsWith('LETTER:'));
  sections.letter.surface = extractSection(responseText, 'LETTER:SURFACE', [...letterEndMarkers, 'LETTER:WADE', 'LETTER:SWIM']);
  sections.letter.wade = extractSection(responseText, 'LETTER:WADE', [...letterEndMarkers, 'LETTER:SWIM']);
  sections.letter.swim = extractSection(responseText, 'LETTER:SWIM', letterEndMarkers);

  // Extract summary
  sections.summary = extractSection(responseText, 'SUMMARY', allMarkers.filter(m => m !== 'SUMMARY'));
  ...
  // Extract path sections at all depths
  const pathEndMarkers = allMarkers.filter(m => !m.startsWith('PATH:'));
  sections.path.surface = extractSection(responseText, 'PATH:SURFACE', [...pathEndMarkers, 'PATH:WADE', 'PATH:SWIM', 'PATH:ARCHITECTURE']);
  sections.path.wade = extractSection(responseText, 'PATH:WADE', [...pathEndMarkers, 'PATH:SWIM', 'PATH:ARCHITECTURE']);
  sections.path.swim = extractSection(responseText, 'PATH:SWIM', [...pathEndMarkers, 'PATH:ARCHITECTURE']);
  sections.path.architecture = extractSection(responseText, 'PATH:ARCHITECTURE', pathEndMarkers);
