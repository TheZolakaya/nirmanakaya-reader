...
export function formatDrawForAI(draws, spreadType, spreadKey, showTraditional) {
  ...
  return draws.map((draw, i) => {
    ...
    let context, positionLens;
    if (isReflect && spreadConfig?.positions?.[i]) {
      context = spreadConfig.positions[i].name;
      positionLens = spreadConfig.positions[i].lens;
    } else {
      context = draw.position !== null ? `Position ${draw.position} ${ARCHETYPES[draw.position]?.name}` : 'Draw';
      positionLens = null;
    }
...
// Post-process Words to the Whys to enforce breathing room
function enforceWordsToWhysBreathing(text) {
  if (!text) return text;

  // Split on signature headers (bold markers like **Too Much Harvest**)
  const signaturePattern = /(\*\*[^*]+\*\*)/g;
  const parts = text.split(signaturePattern).filter(p => p.trim());
  ...
  for (const part of parts) {
    if (part.startsWith('**') && part.endsWith('**')) {
      // This is a signature header
      if (currentSignature) {
        processed.push(currentSignature);
      }
      currentSignature = part;
    } else if (currentSignature) {
      // This is content for the current signature
      let content = part.trim();

      // Break long paragraphs into shorter ones (every 2-3 sentences)
  ...
  // Join signatures with double newlines for clear separation
...
export function parseReadingResponse(responseText, draws) {
  ...
  if (wordsToWhysMatch) {
    sections.wordsToWhys = enforceWordsToWhysBreathing(wordsToWhysMatch[1].trim());
  }
