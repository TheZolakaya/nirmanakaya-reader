...
Domain (POSITION — where this card landed): ${positionName} [${posHouse} House, ${posProcessStage} stage, ${posHorizon} horizon]
Status: ${stat.name} — ${stat.desc}
Process Stage Context: ${positionName} operates at the ${posProcessStage} stage — ${PROCESS_STAGES[posProcessStage]?.description || 'unknown function'}`;
    ...
    // Correction (always included)
    if (correctionText) {
      result += `\nCorrection: ${correctionText}`;
      result += `\nREBALANCER TARGET: ${rebalancerTargetName}. This is the ONLY card to discuss in rebalancer sections.`;
      result += `\nREBALANCER CONTEXT: This correction is happening in the ${positionName} position (${posHouse} House, ${posProcessStage} stage). The rebalancing must address how ${rebalancerTargetName} restores balance specifically within the domain of ${positionName}. Position shapes the correction — the same imbalance means something different in ${positionName} than it would elsewhere.`;
    } else {
      result += `\nNo correction needed (Balanced)`;
    }

    // === SWIM TIER (additional structural context) ===
    if (depth === 'swim' || depth === 'deep') {
      // Gestalt significance (positions 0, 1, 19, 20 operate at meta-level)
