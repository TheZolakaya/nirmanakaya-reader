...
export function generateSpread(count, isReflect = false) {
  // B11 Fix: Cap count at 22 positions (number of archetypes) to prevent undefined positions
...
// DTP: Generate exactly 5 draws with guaranteed unique positions
export function generateDTPDraws() {
  return generateSpread(5, false);
}
...
export function formatDrawForAI(draws, spreadType, spreadKey, showTraditional) {
  ...
  return draws.map((draw, i) => {
    ...
    // Get correction target name for REBALANCER enforcement
...
REBALANCER TARGET: ${rebalancerTargetName}. This is the ONLY card to discuss in rebalancer sections. Do NOT substitute any other card.` : 'No correction needed (Balanced)'}
...
CRITICAL INTERPRETATION RULE: Your interpretation MUST synthesize CARD + POSITION. Don't describe ${trans.name} in the abstract — explain what ${trans.name} means BECAUSE it landed in ${domainContext}. The position shapes the meaning entirely. Ask: "What does this card mean in THIS specific life domain?" The same card in a different position would mean something different. Example: "${trans.name} in your ${domainContext}" = how this energy expresses in that arena of life. The synthesis IS the interpretation — not the card alone.`;
  }).join('\n\n');
}
