...
export function formatDrawForAI(draws, spreadType, spreadKey, showTraditional) {
  ...
  return draws.map((draw, i) => {
    ...
    const rebalancerLine = rebalancerTargetName
      ? `\nREBALANCER TARGET: ${rebalancerTargetName}. When writing [CARD:${i + 1}:REBALANCER:*] sections, discuss ONLY this signature.`
      : '';
...
${correctionText ? `Correction: ${correctionText}. IMPORTANT: Use this exact correction, do not calculate different numbers.` : 'No correction needed (Balanced)'}${rebalancerLine}
