...
export const OPENROUTER_MODEL_IDS = {
  haiku: process.env.OPENROUTER_MODEL_HAIKU || 'deepseek/deepseek-v4-flash',
  sonnet: process.env.OPENROUTER_MODEL_SONNET || 'deepseek/deepseek-v4-flash',
...
export function usdFor(usage, modelId) {
  ...
  if (id.includes('/')) { // an OpenRouter id — no cache-write charge; cache reads at input rate unless the host says otherwise
    const p = OPENROUTER_PRICING[id] || OPENROUTER_PRICING['deepseek/deepseek-v4-flash'];
