...
// Create regex pattern for all terms (word boundaries, CASE SENSITIVE)
// Framework terms should be capitalized when used as concepts (e.g., "Potential" not "potential")
export const HOTLINK_PATTERN = new RegExp(
  `\\b(${SORTED_TERM_KEYS.map(t => t.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')).join('|')})\\b`,
  'g'  // Case-sensitive: only links properly capitalized framework terms
);

// Render text with clickable hotlinks for invariant terms
// Supports BOTH:
// 1. Explicit [Term] bracket markers (for AI-controlled precision)
// 2. Case-sensitive matching for capitalized terms (fallback)
// Colors: amber=cards, cyan=structure (house/channel/role), emerald=status, violet=glossary concepts
// When showTraditional=true, card names show as "CardName [Traditional #]"
export const renderWithHotlinks = (text, setSelectedInfo, showTraditional = false) => {
  ...
  // Get traditional name annotation for cards (only archetypes 0-20 have traditional tarot names)
  const getTraditionalAnnotation = (termInfo) => {
    if (!showTraditional || termInfo.type !== 'card') return null;
    const cardData = getComponent(termInfo.id);
    if (cardData?.traditional) {
      // For archetypes (0-20), show number too
      if (termInfo.id <= 20) {
        return ` [${cardData.traditional} ${termInfo.id}]`;
      }
      return ` [${cardData.traditional}]`;
    }
    return null;
  };
  ...
  const ClickableTerm = ({ termInfo, children }) => {
    const traditionalAnnotation = getTraditionalAnnotation(termInfo);
    return (
      <span
      ...
      >
        {children}
        {traditionalAnnotation && <span className="text-zinc-500 text-xs">{traditionalAnnotation}</span>}
      </span>
    );
  };

  // Look up a term in our hotlink dictionary (case-insensitive lookup)
  const lookupTerm = (termName) => {
    // First try exact match
    if (HOTLINK_TERMS[termName]) return HOTLINK_TERMS[termName];
    // Then try case-insensitive
    return HOTLINK_TERMS_LOWERCASE[termName.toLowerCase()];
  };
  ...
  // Combined pattern: matches BOTH [Term] brackets AND capitalized terms
  // Brackets take priority (more explicit), then fall back to case-sensitive word matching
  ...
  markdownParts.forEach((part, partIndex) => {
    ...
    while ((match = combinedPattern.exec(partText)) !== null) {
      // Add text before match
      if (match.index > lastIndex) {
        hotlinkParts.push({ text: partText.slice(lastIndex, match.index), isLink: false });
      }

      // Determine if this was a bracket match [Term] or a word boundary match
      const isBracketMatch = match[1] !== undefined;
      const termName = isBracketMatch ? match[1] : match[2];
      const displayText = termName; // For brackets, we strip the brackets; for words, show as-is

      // Look up the term (case-insensitive for brackets, exact for word matches)
      const termInfo = lookupTerm(termName);
      if (termInfo) {
        hotlinkParts.push({ text: displayText, isLink: true, termInfo });
      } else {
        // If bracket but not in dictionary, just show the text (no brackets)
        // If word match but somehow not found, show as-is
        hotlinkParts.push({ text: displayText, isLink: false });
      }
