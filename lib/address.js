...
import { relate } from './geometryEngine.js';
...
// .563: THE DISTANCE FROM WHERE YOU WERE LOOKING — the engine's relation between a pointer and another card, read as a locator's
// line: 1 bit = right beside what you asked about; 2 = across one line, a different room on the same floor; 3 = most of the way
// across; 4 = the far side of your life from where you were looking. Same house = the same room. A meaningful sum = a pair; the
// thing is the axis between them. Between two pointers: 1–2 bits = the field agrees (one thing); 4 = it scatters (probably two).
export function distanceLine(pointerDraw, otherDraw, label = 'the card you asked about') {
  try {
    const r = relate(pointerDraw.transient, otherDraw.transient); if (!r) return null;
    const d = r.displacement?.computable ? r.displacement.distance : null;
    const bits = [];
    if (d != null) bits.push(d <= 1 ? 'right beside it — one door away' : d === 2 ? 'across one line — a different room on the same floor' : d === 3 ? 'most of the way across' : 'the far side of their life from where they were looking');
    if (r.sameHouse) bits.push(`the same room (${r.sameHouse})`);
    if (r.sameStage) bits.push(`the same stage (${r.sameStage})`);
    if (r.meaningfulSum) bits.push(`a pair (sum ${r.meaningfulSum}) — the thing is the axis between them`);
    if (r.majorKinship?.length) bits.push(`kin through ${r.majorKinship.map((k) => k.name).filter(Boolean).join(', ')}`);
    if (!bits.length) return null;
    return `  THE DISTANCE from ${label} (${getComponent(otherDraw.transient)?.name}): ${bits.join(' · ')}.`;
  } catch { return null; }
}
